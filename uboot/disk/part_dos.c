...
static int test_block_type(unsigned char *buffer)
{
	...
	if((buffer[DOS_PART_MAGIC_OFFSET + 0] != 0x55) ||
	    ...
	    printf("test_part_dos DOS_PART_MAGIC_OFFSET fail!\n");
		return (-1);
	} /* no DOS Signature at all */

	printf("test_part_dos DOS_PART_MAGIC_OFFSET ok!\n");
			...
			if (!slot &&
			    (strncmp((char *)&buffer[DOS_PBR_FSTYPE_OFFSET],
			     ...
			     strncmp((char *)&buffer[DOS_PBR32_FSTYPE_OFFSET],
				     "FAT32", 5) == 0)) {
				printf("test_part_dos DOS_PBR_FSTYPE_OFFSET ok!\n");
				if (strncmp((char *)&buffer[DOS_PBR_FSTYPE_OFFSET],
				     "FAT", 3) == 0){
					printf("dos fat!\n");
				}

				if (strncmp((char *)&buffer[DOS_PBR32_FSTYPE_OFFSET],
				     "FAT32", 5) == 0){
					printf("dos fat32!\n");
				}
				...
				printf("test_part_dos DOS_PBR_FSTYPE_OFFSET fail!\n");
				return -1;
			}
		}
	}
...
int test_part_dos (block_dev_desc_t *dev_desc)
{
	...
	if (dev_desc->block_read(dev_desc->dev, 0, 1, (ulong *) buffer) != 1){
		printf("test_part_dos read fail!\n");
		return -1;
	}else{
		printf("test_part_dos read ok!\n");
	}
	
	if (test_block_type(buffer) != DOS_MBR) {
		printf("test_part_dos DOS_MBR fail!\n");
		return -1;
	}else{
		printf("test_part_dos DOS_MBR ok!\n");
	}
...
{
	int ret = get_partition_info_extended(dev_desc, 0, 0, 1, part, info, 0);
