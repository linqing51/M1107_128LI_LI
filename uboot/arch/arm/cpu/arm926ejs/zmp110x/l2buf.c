...
T_VOID l2_init(T_VOID)
{
   ...
   memset((void *)s_L2_DMA, 0, sizeof(s_L2_DMA));
    //s_bFracStart = ZMP_FALSE; //luheshan
}
...
static T_VOID l2_dma(T_U32 ram_addr, T_U8 buf_id, T_U32 tran_byte, T_U8 tran_dir, T_BOOL bIntr)
{
    ...
    s_L2_DMA[buf_id].bNeedFrac = ZMP_FALSE;
    if(s_L2_DMA[buf_id].nFracDataLen != 0)
    {
        s_L2_DMA[buf_id].bNeedFrac = ZMP_TRUE;
...
//    MMU_Clean_Invalidate_Dcache();//luheshan
    ...
    {
        s_L2_DMA[buf_id].bStartDMA = ZMP_FALSE;
        s_L2_DMA[buf_id].bStartFrac = ZMP_TRUE;
    ...
    s_L2_DMA[buf_id].bStartDMA = ZMP_TRUE;
...
static T_BOOL l2_wait_dma_finish(T_U8 buf_id)
{
    ...
    if(s_L2_DMA[buf_id].bStartFrac)
    {
        ...
        s_L2_DMA[buf_id].bStartFrac = ZMP_FALSE;
...
T_U8 l2_alloc(DEVICE_SELECT dev_slct)
{
    ...
    //enable buffer
    l2_combuf_ctrl(slct_id, ZMP_TRUE);
...
T_VOID l2_free(DEVICE_SELECT dev_slct)
{
    ...
    s_L2_DMA[free_id].bIntr= ZMP_FALSE;
    s_L2_DMA[free_id].callback_func = ZMP_NULL;
    if (s_L2_DMA[free_id].bStartDMA || s_L2_DMA[free_id].bStartFrac)
    {
        s_L2_DMA[free_id].bStartDMA = ZMP_FALSE;
        s_L2_DMA[free_id].bStartFrac = ZMP_FALSE;
    }
    ...
    //disable buffer
    l2_combuf_ctrl(free_id, ZMP_FALSE);
    //irq_unmask();
