...
extern int zmp_arch_interrupt_init (void);
extern void zmp_do_irq (int hw_irq);
...
void zmp_do_irq (int hw_irq)
{
	int irq = hw_irq; // next_irq();

	// printf("hw_irq=%d\n", hw_irq);
	IRQ_HANDLER[irq].m_func(IRQ_HANDLER[irq].m_data);
}

void zmp_irq_install_handler (int irq, interrupt_handler_t handle_irq, void *data)
{
...
int zmp_arch_interrupt_init (void)
{
	int i;

	/* install default interrupt handlers */
	for (i = 0; i < N_IRQS; i++)
		zmp_irq_install_handler(i, default_isr, (void *)i);
