	...
	IPaddr_t targetIP)
{
	...
	debug_cond(DEBUG_DEV_PKT, "NetOurEther %pM\n", NetOurEther);
	debug_cond(DEBUG_DEV_PKT, "sourceIP %pI4\n", &sourceIP);
	debug_cond(DEBUG_DEV_PKT, "targetEther %pM\n", targetEther);
	debug_cond(DEBUG_DEV_PKT, "targetIP %pI4\n", &targetIP);

	pkt = NetArpTxPacket;  // (len=14 + 28 )
...
void ArpReceive(struct ethernet_hdr *et, struct ip_udp_hdr *ip, int len)
{
	...
	debug_cond(DEBUG_NET_PKT, "Got ARP22\n");
	if (NetReadIP(&arp->ar_tpa) != NetOurIP)
		return;
	debug_cond(DEBUG_NET_PKT, "Got ARP33\n");
	...
	case ARPOP_REPLY:		/*  ping arp reply ok*/
		...
		if (reply_ip_addr == NetArpWaitReplyIP) {
			...
			//printf("net_get_arp_handler1:0x%x\n", net_get_arp_handler());
			...
			memcpy(((struct ethernet_hdr *)NetTxPacket)->et_dest,
				&arp->ar_sha, ARP_HLEN);
			//printf("arp_receive:NetTxPacket:0x%x, NetArpWaitTxPacketSize:%d\n", NetTxPacket, NetArpWaitTxPacketSize);
			NetSendPacket(NetTxPacket, NetArpWaitTxPacketSize);
			//printf("net_get_arp_handler2:0x%x\n", net_get_arp_handler());
			//printf("net_get_arp_handler3:0x%x\n", net_get_arp_handler());
			//printf("net_get_arp_handler4:0x%x\n", net_get_arp_handler());
