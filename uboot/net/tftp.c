...
#define TFTP_BLOCK_SIZE		512 // 1468 // 1536 // 512
	    ...
	    unsigned len)
{
	...
	case TFTP_ACK:
		...
		if (TftpWriting) {
			if (TftpFinalBlock) {
				printf("tftp_complete TftpFinalBlock\n");
				tftp_complete();
			} else {
				/*
...
#endif
		if (len < TftpBlkSize) {
			//printf("if tftp_complete len:0x%x < TftpBlkSize:0x%x\n", len, TftpBlkSize);
			tftp_complete();
		}
		//else{
		//	printf("else tftp_complete len:0x%x < TftpBlkSize:0x%x\n", len, TftpBlkSize);
		//}
