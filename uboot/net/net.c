...
static uchar PktBuf[(PKTBUFSRX+1) * PKTSIZE_ALIGN + PKTALIGN]; // all len+1536*5+64 =7744
...
static void NetInitLoop(void)
{
	...
	if (env_changed_id != env_id) {
		...
		debug_cond(DEBUG_INT_STATE, "NetInitLoop !=\n"); // entry ok
	}

	// get current eth mac address
	if (eth_get_dev()) // get eth_current net device, /include/net.h
	{
		memcpy(NetOurEther, eth_get_dev()->enetaddr, 6); // get our mac address
		debug_cond(DEBUG_INT_STATE, "eth_get_dev\n");  // entry ok
		debug_cond(DEBUG_INT_STATE, "NetInitLoop NetOurEther in environment is  %pM\n", NetOurEther);
	}
...
void net_init(void)
{
	...
	if (first_call) {
		...
		NetTxPacket = &PktBuf[0] + (PKTALIGN - 1); // PKTALIGN=64
		//debug("NetTxPacket1:0x%x\n", NetTxPacket);
		NetTxPacket -= (ulong)NetTxPacket % PKTALIGN;
		//debug("NetTxPacket2:0x%x\n", NetTxPacket);
		
		// PKTBUFSRX=4
		for (i = 0; i < PKTBUFSRX; i++) {
			NetRxPackets[i] = NetTxPacket + (i + 1) * PKTSIZE_ALIGN; // ?????
			//printf("NetRxPackets[%d]:0x%x\n", i, NetRxPackets[i]);
		}
		
		ArpInit();
		net_clear_handlers();  // entry ok

		/* Only need to setup buffer pointers once. */
		first_call = 0;
	}

	NetInitLoop();  // ???? i don't know
}
...
int NetLoop(enum proto_t protocol)
{
	...
	// flag bootstage ethernet start
	bootstage_mark_name(BOOTSTAGE_ID_ETH_START, "eth_start");

	// initial net
	net_init();

	// eth_is_on_demand_init function always return 1 (include/net.h)
	if (eth_is_on_demand_init() || protocol != NETCONS) {
		// stop net 
		eth_halt();
		eth_set_current();
		if (eth_init(bd) < 0) {// retry initial net mac and phy controller
			eth_halt();
			return -1;
		}
	} else
		eth_init_state_only(bd); // get net device status, include/net.h

restart:
#ifdef CONFIG_USB_KEYBOARD   // CONFIG_USB_KEYBOARD=n
	net_busy_flag = 0;
#endif
	net_set_state(NETLOOP_CONTINUE); //  first set netloop status as continue
	...
	// cmd ping ,net_check_prereq return 0 as ok
	...
	case 0:	// 
		...
		case PING:
			ping_start(); // from here ,we can entry ping start
			break;
#endif
#if defined(CONFIG_CMD_NFS)
		case NFS:
...
NetSetTimeout(ulong iv, thand_f *f)
{
	if (iv == 0) {
		...
		timeDelta = iv * CONFIG_SYS_HZ / 1000;  // #define CONFIG_SYS_HZ  		(1000)
	}
}
...
NetReceive(uchar *inpkt, int len)
{
	...
	debug_cond(DEBUG_NET_PKT, "Receive from protocol2 0x%x\n", eth_proto);
	...
	debug_cond(DEBUG_NET_PKT, "Receive from protocol3 0x%x\n", eth_proto);
	switch (eth_proto) {
	...
	debug_cond(DEBUG_NET_PKT, "Receive from protocol4 0x%x\n", eth_proto);
}
...
NetSetEther(uchar *xet, uchar * addr, uint prot)
{
	...
	//printf("NetSetEther addr in environment is  %pM\n", addr);
	memcpy(et->et_src, NetOurEther, 6);
	//printf("NetSetEther NetOurEther in environment1 is  %pM\n", NetOurEther);
	//printf("NetSetEther NetOurEther in environment2 is  %pM\n", NetOurEther);
	//printf("NetSetEther NetOurEther in environment3 is  %pM\n", NetOurEther);
	//printf("NetSetEther NetOurEther in environment4 is  %pM\n", NetOurEther);
	//printf("NetSetEther NetOurEther in environment5 is  %pM\n", NetOurEther);
	if ((myvlanid & VLAN_IDMASK) == VLAN_NONE) {
