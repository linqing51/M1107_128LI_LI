...
unsigned long flash_init (void)
{
	...
	fli->size = flash->size;
	fli->sector_count = flash->size/flash->erase_size; // notes:use erase size , not sector size
	fli->flash_id = flash->flashid;
	//printf("%s, flash->size:0x%x\n", __func__, flash->size);
	//printf("%s, flash->erase_size:0x%x\n", __func__, flash->erase_size);
	//printf("%s, fli->sector_count:0x%x\n", __func__, fli->sector_count);
	//printf("%s, fli->flash_id:0x%x\n", __func__, fli->flash_id);
...
void wait_ao_module_rdy(void)
{
	...
	while(!((REG32_EFUSE(SYSINT_STATUS_REG)>>17)&0x1));
}
...
void write_ao_module_reg(unsigned         long addr,unsigned long data)
{
	...
	reg_value = REG32_EFUSE(ALWAYS_ON_PMU_CTRL0_REG);
	reg_value |= ((0x2<<18) | ((addr&0x7)<<14) | ((data&0x3fff)<<0) | (0x1<<21));
	reg_value &= ~(0x1<<17);
	REG32_EFUSE(ALWAYS_ON_PMU_CTRL0_REG) = reg_value;
...
unsigned long read_ao_module_reg(unsigned long addr)
{
	...
	reg_value1 = REG32_EFUSE(ALWAYS_ON_PMU_CTRL0_REG);
	reg_value1 |= ((0x2<<18) | (0x1<<17) | ((addr&0x7)<<14) | (0x1<<21));
	REG32_EFUSE(ALWAYS_ON_PMU_CTRL0_REG) = reg_value1;

	wait_ao_module_rdy();

	reg_value2 = REG32_EFUSE(ALWAYS_ON_PMU_CTRL1_REG);

	disable_ao_module_wr_rd();

	return (reg_value2&0x3fff);
...
void efuse_from_always_on_trimming_for_voltage_test(unsigned long trimming)
{
	...
	trimming=trimming+0X10;//efuse valid and trimming

	reg_value = read_ao_module_reg(AO_ANALOG_CONFIG);

	reg_value &= ~(0x1f<<0);

	reg_value |= (trimming<<0);

	write_ao_module_reg(AO_ANALOG_CONFIG,(reg_value));
...
static int spl_part_parse(const char *const partdef, const char **ret, struct spl_part_info **retpart)
{
...
int spl_device_parse(const char *const mtd_dev)
{
	const char *mtd_id;
	const char *p;
	...
	while (p && (*p != '\0') && (*p != ';')) {
		err = 1;
		if ((spl_part_parse(p, &p, &part) != 0) || (!part))
			break;
...
const void *spl_fdt_offset_ptr(const void *fdt, int offset, unsigned int len)
{
	...
	/* if fdt version >= 0x11 and offset beyond dt struct size range, then report err */
