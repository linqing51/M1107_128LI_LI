...
#ifdef CONFIG_SPINAND_FLASH
					if( spinand_erase_partition(env_flash, CONFIG_BKENV_OFFSET,  CONFIG_BKENV_SIZE)  == -1){
						error("error: erase env partition fail : g_config_env_offset = %d\n", CONFIG_BKENV_OFFSET);
						goto ENV_DEFAULT;
					}
					else{
						if( spinand_write_partition(env_flash, CONFIG_BKENV_OFFSET, CONFIG_BKENV_SIZE, (u8 *)buf, CONFIG_ENV_SIZE) == -1){
							error("error: write env partition fail : g_config_env_offset = %d\n", CONFIG_BKENV_OFFSET);
							goto ENV_DEFAULT;
						}
					}
#else
					ret = spi_flash_erase(env_flash, CONFIG_BKENV_OFFSET, CONFIG_BKENV_SIZE);
					if (ret){
						printf("error:erase env partition fail, g_config_env_size:%d, %d", CONFIG_BKENV_OFFSET, CONFIG_BKENV_SIZE);
						goto ENV_DEFAULT;
					}
					else{
						ret = spi_flash_write(env_flash, CONFIG_BKENV_OFFSET, CONFIG_BKENV_SIZE, buf);
						if (ret){
							printf("error:write env partition fail, g_config_env_size:%d, %d", CONFIG_BKENV_OFFSET, CONFIG_BKENV_SIZE);
							goto ENV_DEFAULT;
						}
					}
#endif
...
CHECK_OK:
	if (himport_r(&env_htab, (char *)ep->data, ENV_SIZE, '\0', 0,
			0, NULL)) {
		gd->flags |= GD_FLG_ENV_READY;
		debug("%s, !himport_r OK env_htab!\n", __func__);
		return 1;
	}

	printf("%s, !himport_r ERR env_htab!\n", __func__);
...
void env_relocate(void)
{
	...
	if (gd->env_valid == 0) {
		...
		env_relocate_spec(); // uboot240/common/env_sf.c
