			      ...
			      char * const argv[])
{
	...
	if (argc == 4) {
		copy_filename(filename, argv[3], sizeof(filename));
		#ifdef CONFIG_SPINAND_FLASH
		check_filename_withoob(filename);
		#endif
	}
...
void read_envbk_spec(void)
{
	...
	flash = spinand_flash_probe(CONFIG_ENV_SPI_BUS, CONFIG_ENV_SPI_CS,
			CONFIG_ENV_SPI_MAX_HZ, CONFIG_ENV_SPI_MODE);
	if (!flash) {
		set_default_env("!spinand_flash_probe() failed");
		return;
	}

	printf("%s, line:%d\n", __func__, __LINE__);
    ret = spinand_read_partition(flash, CONFIG_BKENV_OFFSET, CONFIG_ENV_PARTITION_SIZE, buf, CONFIG_ENV_SIZE);
	if (ret == -1) {
...
void read_envbk_spec(void)
{
    ...
    ret = spi_flash_read(flash,
		CONFIG_BKENV_OFFSET, CONFIG_BKENV_SIZE, buf);
	if (ret) {
...
int do_format(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{ 
#if 0
    char *cfgcmd  = "readcfg";
	...
	debug("%s,make bootargs!\n", __func__);
	run_command(cfgcmd, 0);
	cmdline = getenv("bootargs");
	cmdroot = getenv("mtd_root");
	proot 	= strstr(cmdline, root); // search "/dev/mtdblock" string
	if(proot != NULL){
		memcpy(proot, cmdroot, 14);
		//debug("%s, proot=0x%x\n", __func__, proot);
		debug("%s, proot:%s, cmdroot:%s\n", __func__, proot, cmdroot);
	}else{
		puts("format search root mtdblock failed!\n");
	}
#endif

	read_envbk_spec();

    run_command("saveenv;reset;", 0);
