...
extern int check_filename_withoob(unsigned char *filename);
#endif
		...
		char * const argv[])
{
	...
	case 2:	/*
		...
		/* add check addr whether beyond UBOOT mem eare range, protect UBOOT memory */
	...
	case 3:	load_addr = simple_strtoul(argv[1], NULL, 16);
		...
		#ifdef CONFIG_SPINAND_FLASH
		check_filename_withoob(BootFile);
		#endif
		break;
...
static int do_ping(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	NetPingIP = string_to_ip(argv[1]); // get dst ip address
	if (NetPingIP == 0)
		return CMD_RET_USAGE;
...
// ping cmd start
U_BOOT_CMD(
	ping,	2,	1,	do_ping,  // name maxargs repeatable 
