...
#define PART_DEL_TYPE		0
#define PART_DELALL_TYPE	1
#define PART_ADD_TYPE		2
#define PART_DEFAULT_TYPE	3



#define PARTITION_MAX_MANE_LEN	6


//#if defined(MTDPARTS_DEFAULT)
#if defined(CONFIG_DEFAULT_MTDPARTS)
static const char *const mtdparts_default = CONFIG_DEFAULT_MTDPARTS; //MTDPARTS_DEFAULT;
#else
...
static int part_del(struct mtd_device *dev, struct part_info *part)
{
	...
	//printf("dev:%x, current_mtd_dev:%x\n", dev, current_mtd_dev);
	if (dev == current_mtd_dev) {
...
#if 0
static int get_ak_partition_table(char *part_buf)
{
	struct list_head *dentry, *pentry;
	struct mtd_device *dev;
	int part_num = 0, idex= 0;
	struct part_info *part;
	...
	list_for_each(dentry, &devices) {
		...
		list_for_each(pentry, &dev->parts) {
			...
			printf("%2d: %-20s0x%08llx\t0x%08llx\t%d\n",
					...
					part->offset, part->mask_flags);
			part_num++;
			memcpy(&part_buf[idex], part, sizeof(struct part_info));
			idex += sizeof(struct part_info);
		}
	}
		...
		u8 *part_num, struct part_info **part)
{
	...
	list_for_each(dentry, &devices) {
		...
		*dev = list_entry(dentry, struct mtd_device, link);
		list_for_each(pentry, &(*dev)->parts) {
			*part = list_entry(pentry, struct part_info, link);
			if (strcmp((const char *)(*part)->name, (const char *)id) == 0)
				return 0;
			(*part_num)++;
		}
	}
...
int mtdparts_init(void)
{
	 ...
	 */
	if (gd->flags & GD_FLG_ENV_READY) {
		debug("%s, GD_FLG_ENV_READY !\n", __func__);
		parts = getenv("mtdparts");
	} else {
		debug("%s, GD_FLG_ENV_NOREADY !\n", __func__);
		parts = tmp_parts;
		getenv_f("mtdparts", tmp_parts, MTDPARTS_MAXLEN);
	}
		       ...
		       char * const argv[])
{
	if (argc == 2) {
		if (strcmp(argv[1], "default") == 0) {
			...
			setenv("mtdparts", (char *)&mtdparts_default[9]);
			setenv("partition", NULL);
	...
	/* mtdparts add <mtd-dev> <size>[@<offset>] <name>[@<p_type>@<run_addr>]  [ro] */
...
#define PART_ADD_DESC_MAXLEN 64
		...
		len += strlen(argv[4]) + 2;	/* '(' name@p_type ')' */
		...
		//part_num = spl_device_parse(getenv("mtdparts"));
		return 0;
	}
