			...
			int *s_count )
{
	...
	*s_count = 0;
	...
	//debug("%s, addr_first:0x%x, addr_last:0x%x\n", __func__, addr_first, addr_last);
	...
	for (bank=0,info = &flash_info[0];
		...
		//debug("%s, info->flash_id:0x%x\n", __func__, info->flash_id);
		//debug("%s, info->start[0]:0x%x, info->size:0x%x, b_end:0x%x\n", __func__, info->start[0], info->size, b_end);
		debug("%s, info->sector_count:0x%x, s_end:0x%x\n", __func__, info->sector_count, s_end);
		
		for (sect=0; sect < info->sector_count; ++sect) {
			ulong end;	/* last address in current sect	*/

			end = (sect == s_end) ? b_end : info->start[sect + 1] - 1;
			//printf("%s, sect:%d, end:0x%x\n", __func__, sect, end);
			if (addr_first > end)
				continue;
		...
		//printf("%s, s_first[%d]:0x%x, s_last[bank]:0x%x\n", __func__, bank, s_first[bank], s_last[bank]);
