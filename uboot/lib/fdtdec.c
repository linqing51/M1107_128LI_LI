...
static const char * const compat_names[COMPAT_COUNT] = {
	COMPAT(UNKNOWN, "<none>"),
	COMPAT(ZMP_H3D_LCD, "zlgmcu,zmp110x-lcd-ctrl"),
	COMPAT(ZMP_H3D_MCI0, "zlgmcu,zmp110x-mmc0"),
	COMPAT(ZMP_H3D_MCI1, "zlgmcu,zmp110x-mmc1"),
	COMPAT(ZMP_H3D_MCI2, "zlgmcu,zmp110x-mmc2"),
	COMPAT(ZMP_H3D_ETH, "zlgmcu,zmp110x-ethernet"),
		...
		s32 default_val)
{
	...
	debug("%s,line:%d (not found)\n", __func__, __LINE__);
	return default_val;
}
/*  add */
s32 fdtdec_get_valoffset_int(const void *blob, int node, const char *prop_name, int valoffset,
		s32 default_val)
{
	const s32 *cell;
	int len;
...
int fdtdec_next_compatible_subnode_sta(const void *blob, int node, int *depthp)
{
	int len; 
	const char *nodep;
	int i = 0;
	...
	for(i=0; i<10; i++){
		do {
			/* search pannel type sub node */
			node = fdt_next_node(blob, node, depthp);
		} while (*depthp > 1);
		//printf("%s, line:%d, node:0x%x\n", __func__, __LINE__, node);
		nodep = fdt_getprop(blob, node, "status", &len);
		//printf("%s, line:%d, nodeP:%s\n", __func__, __LINE__, nodep);
