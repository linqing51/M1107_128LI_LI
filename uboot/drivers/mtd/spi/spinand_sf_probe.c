 ...
 */
static struct spinand_flash_info zmp_spinand_param_supportlist [] = {
	...
	{ "MX35LF1GE4AB", 0x12c212c2, 0, 2048, 64, 128*1024, 1024, 64, 0, 16, 0, 4, 4, SFLAG_DUAL_READ|SFLAG_QUAD_READ|SFLAG_QUAD_WRITE,},
    { "IS38SML01G1", 0x7f7f21c8, 0, 2048, 64, 128*1024, 1024, 64, 0, 8, 8, 4, 8, SFLAG_DUAL_READ|SFLAG_QUAD_READ|SFLAG_QUAD_WRITE,},
	{ "F35SQA001G", 0xcd7171cd, 0, 2048, 64, 128*1024, 1024, 64, 0, 12, 4, 4, 8, SFLAG_DUAL_READ|SFLAG_QUAD_READ|SFLAG_QUAD_WRITE,},
};
...
static struct spinand_flash *spinand_flash_validate_params(struct spi_slave *spi, u8 *idcode)
{
	...
	debug("zmpspi nand ID: 0x%08x\n", jedec);
	for (i = 0, info = zmp_spinand_param_supportlist;i < ARRAY_SIZE(zmp_spinand_param_supportlist);i++, info++) {
		if (info->jedec_id == jedec) {
			break;
		}
	}

	if(i == ARRAY_SIZE(zmp_spinand_param_supportlist))
	{
	...
	else
	{
	    ...
	    spinand_flash_param.plane_blk_num = info->n_blocks;   
	...
	//g_spinand_info = (T_SPINAND_INFO *)zmp_spinand_supportlist + flash->oob_idex;
...
{	
	...
	blockcnt = partition_size/flash->erase_size;
