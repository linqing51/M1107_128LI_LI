...
RETRY_READ:
	...
	if(dcrc != be32_to_cpu(scrc)){
		...
		else if(spl_image.os == IH_OS_U_BOOT){//uboot crc error and uboot bk no exist, fail
		...
		else if(spl_image.os == IH_OS_LINUX){//kernel crc error  and read kernelbk  partition
	...
	partition_size = run_len;
	//get DTB 
	//printf("flash_offset:0x%x and partition_size:0x%x\n", flash_offset, partition_size);
