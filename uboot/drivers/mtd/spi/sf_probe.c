...
static const struct spi_flash_params spi_flash_params_table[] = {
#ifdef CONFIG_SPI_FLASH_ZMP
	/* ISSI */
	{ "IS25LP064", 0x9d6017, 0x9d60, 64 * 1024, 128, SECT_4K | SFLAG_QUAD_READ | SFLAG_QUAD_WRITE, 0,},
	{ "IS25LP0512", 0x9d601a, 0x9d60, 64 * 1024, 1024, SECT_4K | SFLAG_QUAD_READ | SFLAG_QUAD_WRITE | SFLAG_COM_BIT24, 0,},
	...
	{ "mx25l12805d", 0xc22018, 0, 64 * 1024, 256, SECT_4K | SFLAG_DUAL_READ, 0,},
	...
	{ "gd25q64c", 0xc84017, 0, 64 * 1024, 128, SECT_4K|SFLAG_COM_STATUS3|SFLAG_QUAD_READ|SFLAG_QUAD_WRITE, 0x53},
	...
	*/
	...
	if(flash->bus_width & FLASH_BUS_WIDTH_2WIRE){
		...
		if(params->flags & SFLAG_DUAL_WRITE) {
			xfer_ctl->tx_opcode = CMD_PAGE_PROGRAM_DUAL; // not support dual write
	...
	if(flash->bus_width & FLASH_BUS_WIDTH_4WIRE){
		if(params->flags & SFLAG_QUAD_READ) {
			...
			xfer_ctl->dummy_len = 3; // modify from winbond 32MB spi norflash
		}

		if(params->flags & SFLAG_QUAD_WRITE) {
			xfer_ctl->tx_opcode = CMD_PAGE_PROGRAM_QUAD;  // check ok
			xfer_ctl->txd_bus_width = SPI_XFER_4DATAWIRE;			
			xfer_ctl->txa_bus_width = SPI_XFER_1DATAWIRE;
		}else if(params->flags & SFLAG_QUAD_IO_WRITE) {  
			xfer_ctl->tx_opcode = CMD_PAGE_PROGRAM_4IO;  // not support dual write
			xfer_ctl->txd_bus_width = SPI_XFER_4DATAWIRE;
			xfer_ctl->txa_bus_width = SPI_XFER_4DATAWIRE;
		}
	}
	// debug("xfer_ctl->tx_opcode[0x%x], xfer_ctl->rx_opcode[0x%x]\n", xfer_ctl->tx_opcode, xfer_ctl->rx_opcode);
	return 0;
}
...
static struct spi_flash *spi_flash_validate_params(struct spi_slave *spi, u8 *idcode)
{
			...
			if ((params->jedec & 0xFFFF) == jedec) {
				...
				*/
				if (params->jedec == 0xc84017) {
					...
					//printf("zmpspi flash VERSION: 0x%x\n", id_ver_c);	
				}
	...
	debug("spi flash. bus_width:4\n");
#else
	flash->bus_width = FLASH_BUS_WIDTH_1WIRE;
	debug("spi flash. bus_width:1\n");
#endif
	...
	if (flash->size > SPI_FLASH_16MB_BOUN) {
		...
		debug("spi flash. CONFIG_SPI_FLASH_BAR:bank_rd_cmd:0x%x\n", flash->bank_read_cmd);
		if (spi_flash_read_common(flash, &flash->bank_read_cmd, 1, &curr_bank, 1)) {
			debug("SF: fail to read bank addr register\n");
			return NULL;
		}
		
		flash->bank_curr = curr_bank & 0x3;
	} else {
	...
	// intial spi operation data wire and cmd
...
//#ifndef CONFIG_SPL_BUILD
	...
	if (flash->size > SPI_FLASH_16MB_BOUN) {
		debug("SF: Warning - Fash size Beyond 16MiB accessible\n");
		...
		if (flash->flags & SFLAG_COM_BIT24) {
			...
			if (flash->flashid == 0x9d601a) {
				int ext_addr = 0;
				if (spi_flash_read_common(flash, &flash->bank_read_cmd, 1, &ext_addr, 1)) {
				...
				if(ext_addr & 0x80) {
					...
					ext_addr &= 0x3;
					if (spi_flash_write_common(flash, &cmd, 1, &ext_addr, 1)) {
			...
			if (regval&(0x1<<fsr->b_ap)){
				...
				/* software reset spi norflash enter 3B address mode */
				...
				/* for delay 30us for reset device complete */
				for (i=0; i<3000; i++) {
					;
				}
			}else {
