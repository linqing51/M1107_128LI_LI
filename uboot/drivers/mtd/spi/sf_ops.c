...
static int spi_flash_cmd_bankaddr_write(struct spi_flash *flash, u8 bank_sel)
{
	...
	spi_flash_cmd_wait_ready(flash,SPI_FLASH_SECTOR_ERASE_TIMEOUT);
...
int spi_flash_cmd_erase_ops(struct spi_flash *flash, u32 offset, size_t len)
{
	...
	if (offset % erase_size || len % erase_size) {
		debug("SF_ Erase offset/length not multiple of erase size:0x%x\n", erase_size);
		return -1;
	}
	...
	unsigned long last_update = get_timer(0); // add for 
	while (len) {
		if (get_timer(last_update) > 100) {
			printf("   \rEraseing, %02d%%    %lu B/s",
			       100 - (100*(end - offset))/end,
