...
T_GPIO_ATTR_CFG_FUNC_PULL zmp_gpio_attr[GPIO_NUMBER+1] = {
	{ZMP_GPIO_0,  
...
T_GPIO_SHAREPIN_CFG_AS_GPIO zmp_gpio_sharepin_as_gpio[GPIO_NUMBER+1] = {
	{ZMP_GPIO_0,  
...
int gpio_set_pull_up_r(const unsigned long pin, int enable)
{
    ...
    for(i = 0; i < (GPIO_NUMBER+1); i++){
        if(pin == zmp_gpio_attr[i].gpio_index){
			if(zmp_gpio_attr[i].gpio_pupd_en_sel_valid == CFG_VALID){
				reg_data = zmp_gpio_attr[i].gpio_pupd_en_reg;
				regvalue = __raw_readl(reg_data);
				regvalue &= ~(zmp_gpio_attr[i].gpio_pupd_en_bitmask << zmp_gpio_attr[i].gpio_pupd_en_bitstart);
				if(enable){
					/* enable gpio pupd enable */
					if(zmp_gpio_attr[i].gpio_pupd_en_enable_set){
						regvalue |= (0x1 << zmp_gpio_attr[i].gpio_pupd_en_bitstart);
					}else{
						regvalue &= ~(0x1 << zmp_gpio_attr[i].gpio_pupd_en_bitstart);
					}
			...
			if(zmp_gpio_attr[i].gpio_pupd_sel_sel_valid == CFG_VALID){
				reg_data = zmp_gpio_attr[i].gpio_pupd_sel_reg;
				regvalue = __raw_readl(reg_data);
				regvalue &= ~(zmp_gpio_attr[i].gpio_pupd_sel_bitmask << zmp_gpio_attr[i].gpio_pupd_sel_bitstart);

				/* set gpio pu according to gpio pu set */
				if(zmp_gpio_attr[i].gpio_pupd_sel_pu_set){
					regvalue |= (0x1 << zmp_gpio_attr[i].gpio_pupd_sel_bitstart);
				}else{
					regvalue &= ~(0x1 << zmp_gpio_attr[i].gpio_pupd_sel_bitstart);
				}
				__raw_writel(regvalue, reg_data);
...
int gpio_set_pin_as_gpio (unsigned int pin)
{
    ...
    for(i = 0; i < (GPIO_NUMBER+1); i++){
        if(pin == zmp_gpio_sharepin_as_gpio[i].gpio_index){
			if(zmp_gpio_sharepin_as_gpio[i].gpio_sharepin_sel_valid == CFG_VALID){
				reg_data = zmp_gpio_sharepin_as_gpio[i].gpio_sharepin_sel_reg;
				regvalue = __raw_readl(reg_data);
				regvalue &= ~(zmp_gpio_sharepin_as_gpio[i].gpio_sharepin_sel_bitmask << zmp_gpio_sharepin_as_gpio[i].gpio_sharepin_sel_bitstart);
				regvalue |= (zmp_gpio_sharepin_as_gpio[i].gpio_sharepin_value << zmp_gpio_sharepin_as_gpio[i].gpio_sharepin_sel_bitstart);
				__raw_writel(regvalue, reg_data);
			}else{
