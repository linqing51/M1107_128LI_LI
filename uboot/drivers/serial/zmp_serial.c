...
#include <asm/io.h>
#include "zmp_serial.h"

DECLARE_GLOBAL_DATA_PTR;

#ifdef CONFIG_SERIAL1
#define UART_NR	zmp_uart0

#elif defined(CONFIG_SERIAL2)
#define UART_NR	zmp_uart1
...
{
	return (dev_index == zmp_uart0) ? UART0_BASE_ADDR : UART1_BASE_ADDR;
}

static inline unsigned long serial_l2buffer_base(unsigned int dev_index)
{
	return (dev_index == zmp_uart0) ? UART0_L2_RX_BUF_BASE : UART1_L2_RX_BUF_BASE;
}
...
int zmpserial_set_baudrate(const int dev_index,unsigned long baudrate)
{
    ...
    asic_freq = get_asic_freq();
    unsigned int baud_target = asic_freq/baudrate - 1;
...
int _serial_getc(const int dev_index)
{
	...
	while(1)
	{
		...
		if (l2_uartbuffer_offset != reg) {
			data = readb(l2_rxbuffer_base + l2_uartbuffer_offset++);
...
struct serial_device zmp_serial0_device =
INIT_AK_SERIAL_STRUCTURE(0, "akser0");
DECLARE_AK_SERIAL_FUNCTIONS(1);
struct serial_device zmp_serial1_device =
INIT_AK_SERIAL_STRUCTURE(1, "akser1");

__weak struct serial_device *default_serial_console(void)
{
#if defined(CONFIG_SERIAL1)
	return &zmp_serial0_device;
#elif defined(CONFIG_SERIAL2)
	return &zmp_serial1_device;
#else
#error "CONFIG_SERIAL? missing."
#endif
}

void zmp_serial_initialize(void)
{
	serial_register(&zmp_serial0_device);
	serial_register(&zmp_serial1_device);
}
