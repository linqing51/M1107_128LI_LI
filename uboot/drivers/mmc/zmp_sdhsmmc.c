...
enum zmp_mmc_cd_mode g_mmc_plug_mod;
unsigned int g_mmc_plug_gpio_index = 0;

struct zmp_sdhsmmc_data *g_pCurSdDevice = ZMP_NULL;

static bool send_cmd(struct mmc *mmc, unsigned char cmd_index, unsigned char resp ,unsigned int arg);
static int zmp_mmc_send_cmd(struct mmc *mmc, struct mmc_cmd *cmd, struct mmc_data *data);

static struct mmc sdhsmmc_dev[3];
static struct zmp_sdhsmmc_data sdhsmmc_dev_data[3];
 ...
 */
static int zmp_mmc_setup_gpio_in(int gpio, const char *label)
{
 ...
 *
 * get gpio pin input level for sd card wp or cd detect
 * @author CaoDonghua
 * @date 2016-09-30
...
static int zmp_mck_get_tfplug_status(void)
{
	...
	if((status[0] == 0x0) && (status[0] == status[1]) && (status[1] == status[2])){
		debug("%s, line:%d mck det TF card ok!\n", __func__, __LINE__);
		return 0;
	}
 ...
 * @retval  return low level 0
 */
static int zmp_mmc_getcd(struct mmc *mmc)
{
	
#ifdef TF_DET_USE_GPIO
		int cd_gpio = ((struct zmp_sdhsmmc_data *)mmc->priv)->cd_gpio;
		return zmp_gpio_get_value(cd_gpio);
#endif
	
#ifdef TF_DET_USE_MCK
		return zmp_mck_get_tfplug_status();
#endif
	return 0; 
}
 ...
 * @retval  return low level 0
 */
static int zmp_mmc_getwp(struct mmc *mmc)
{
	int wp_gpio = ((struct zmp_sdhsmmc_data *)mmc->priv)->wp_gpio;
	return zmp_gpio_get_value(wp_gpio);
}
 ...
 */
static int zmp_mmc_sd_sharepin_cfg(int mmcdev_index)
{
	unsigned int value = 0;
	unsigned int mci_sel = 0;
	
	if (mmcdev_index == 0) { // mmc/sd
		printf("sd1 share pin!\n");
        // todo
        /*as for platform,only use MCI0_D[0],that's say,we only have 1bit bus_width*/
		value = REG32(SHARE_PIN_CFG1_REG);
		value &= ~((0x3<<18)|(0x3<<20)|(0x3<<22)); 
		value |= ((0x1<<18)|(0x1<<20)|(0x1<<22));
		REG32(SHARE_PIN_CFG1_REG) = value;

	}else if (mmcdev_index == 1) { // sdio
		...
		*/
		...
		zmp_group_config(ePIN_AS_SDIO);

		/* set gpio77 sharepin as gpio and as sd power ctrl, output low */
		gpio_set_pin_as_gpio(ZMP_GPIO_77);
		gpio_direction_input(ZMP_GPIO_77);

		/* set gpio81 sharepin as gpio and as sd_det */
		gpio_set_pin_as_gpio(ZMP_GPIO_81);
		gpio_direction_output(ZMP_GPIO_81, ZMP_GPIO_OUT_LOW);
#endif
 ...
 * @retval  return a non-zero if failed
 */
static int zmp_mmc_sd_system_clk(int mmcdev_index)
{
	unsigned int value = 0;

	if (mmcdev_index == 0) {
		value = readl(CLOCK_CTRL_REG);
		value &= ~(0x1<<1);	// enable mci0 module clock
		writel(value, CLOCK_CTRL_REG);
	}else if (mmcdev_index == 1) {
		printf("set sd2 open clk!\n");
		value = readl(CLOCK_CTRL_REG);
		value &= ~(0x1<<2); // enable mci1 module clock
		writel(value, CLOCK_CTRL_REG);
	}else if (mmcdev_index == 2) {
		value = readl(CLOCK_CTRL2_REG);
		value &= ~(0x1<<3); // enable mci2 module clock
 ...
 * @retval return a non-zero if failed
 */
static int zmp_mmc_board_init(struct mmc *mmc)
{
	int devindex;
	unsigned int value = 0;
	
	devindex = ((struct zmp_sdhsmmc_data *)mmc->priv)->dev_index;

#if 0
	/* config selected mmc/sd bus interface share pin */ 
	if (zmp_mmc_sd_sharepin_cfg(devindex) == -1) {
    ...
    if (zmp_mmc_sd_system_clk(devindex) == -1) {
 ...
 * @return void
 */
static void zmp_set_clock(struct mmc *mmc, unsigned int clk, unsigned int asic, bool pwr_save)
{
    unsigned char clk_div_l,clk_div_h;
    unsigned int  asic_freq,reg_value,tmp;
	struct zmp_sdhsmmc_data *priv_data = (struct zmp_sdhsmmc_data *)mmc->priv;
 ...
 * @retval  ZMP_TRUE: CMD sent successfully
 * @retval  ZMP_FALSE: CMD sent failed
 */
static bool send_cmd(struct mmc *mmc, unsigned char cmd_index, unsigned char resp, unsigned int arg)
{
 ...
 * @retval  ZMP_TRUE: ACMD sent successfully
 * @retval  ZMP_FALSE: ACMD sent failed
 */
...
* @retval  ZMP_TRUE: sd controller is transferring
* @retval  ZMP_FALSE: sd controller is not transferring
*/
...
* @retval  ZMP_TRUE: send  successfully
* @retval  ZMP_FALSE: send failed
*/
...
* @retval  ZMP_TRUE: receive  successfully
* @retval  ZMP_FALSE: receive failed
*/
 ...
 * @retval  ZMP_TRUE: transfer successfully
 * @retval  ZMP_FALSE: transfer failed
 */
...
* @retval return a non-zero if failed
*/
static int zmp_mmc_read_data(struct mmc *mmc, struct mmc_data *data, char *buf, unsigned int size)
{
	int ret = 0;
...
* @retval return a non-zero if failed
*/
static int zmp_mmc_write_data(struct mmc *mmc, struct mmc_data *data, char *buf, unsigned int size)
{
	int ret = 0;
...
* @retval return a non-zero if failed
*/
static int zmp_mmc_send_cmd(struct mmc *mmc, struct mmc_cmd *cmd,
			struct mmc_data *data)
{
    ...
    if(cmd->cmdidx == SD_CMD(25)){
        if (send_acmd(mmc, 23, SD_SHORT_RESPONSE, data->blocks) == ZMP_FALSE ){
	...
	if (data && (data->flags & MMC_DATA_READ)) {
		ret = zmp_mmc_read_data(mmc, data, data->dest, data->blocksize * data->blocks);
	} else if (data && (data->flags & MMC_DATA_WRITE)) {
		ret = zmp_mmc_write_data(mmc, data,  (char *)data->src, data->blocksize * data->blocks);
	}
...
static void zmp_mmc_set_ios(struct mmc *mmc)
{
	unsigned int arg_value = 0;

	if (mmc->iosset_flag == 0x1) {
		/* SD_POWER_SAVE_ENABLE */
		zmp_set_clock(mmc, mmc->clock, get_asic_freq(), SD_POWER_SAVE_ENABLE);
		...
		printf("mmc->iosset_flag no valid!\n");
	}
...
* @date 2016-09-26
* @param[in] mmc  mmc device handle.
* @return int return sd  interface and controller init success or failed
* @retval returns zero on success
* @retval return a non-zero if failed
*/
static int zmp_mmc_init_setup(struct mmc *mmc)
{
	/* initial sd interface sharepin 
	*   and open this sd system module clock 
	*/ 
	zmp_mmc_board_init(mmc);

	/* initial sd interface default clock from get current asic clock
	*   and open this sd system module clock 
	*   and enable power save
	*/ 
	zmp_set_clock(mmc, SD_IDENTIFICATION_MODE_CLK, get_asic_freq(), SD_POWER_SAVE_DISABLE);
	mdelay(100);
...
/**
* @brief mmc/sd controller init
*
*  init mmc/sd controller features
* @author CaoDonghua
* @date 2016-09-26
...
* @return int return mmc/sd controller init success or failed
* @retval returns zero on success
* @retval return a non-zero if failed
*/
int zmp_sdhsmmc_init(int dev_index, uint host_caps_mask, uint f_max, int cd_gpio,
		int wp_gpio)
{
	struct mmc *mmc = &sdhsmmc_dev[dev_index];
	struct zmp_sdhsmmc_data *priv_data = &sdhsmmc_dev_data[dev_index];

	/* set mmc/sd controller support capacity,We use 1bit bus_width */
	uint host_caps_val = MMC_MODE_HS_52MHz | MMC_MODE_HS | MMC_MODE_HC;

	sprintf(mmc->name, "mmc_sd");
	mmc->send_cmd = zmp_mmc_send_cmd;
	mmc->set_ios  = zmp_mmc_set_ios;
	mmc->init 	  = zmp_mmc_init_setup;
	mmc->priv 	  = priv_data;

	/* select the dev mmc bus register base address */
	switch (dev_index) {
	case 0:
		priv_data->base_addr = (struct akhsmmc *)ZMP_SDHSMMC1_BASE;
		break;
	case 1:
		priv_data->base_addr = (struct akhsmmc *)ZMP_SDHSMMC2_BASE;
		break;
	case 2:
		priv_data->base_addr = (struct akhsmmc *)ZMP_SDHSMMC3_BASE;
		break;
	default:
		priv_data->base_addr = (struct akhsmmc *)ZMP_SDHSMMC1_BASE;
		break;
	...
	//debug("%s, line:%d, dev_index:0x%x, sd_base:0x%x\n", __func__, __LINE__, dev_index, priv_data->base_addr);
	priv_data->dev_index = dev_index;

	/* check if has cd detect pin, and set match handle function, cd_gpio=ZMP_GPIO_57 */
	priv_data->cd_gpio = zmp_mmc_setup_gpio_in(cd_gpio, "mmc_cd");
	if (priv_data->cd_gpio != -1) {
		debug("yes cd_gpio=%d\n", priv_data->cd_gpio);
		mmc->getcd = zmp_mmc_getcd;
	}

	/* check if has cd detect pin, and set match handle function, cd_gpio=ZMP_GPIO_57 
	*  if no wp , we set wp_gpio = 0xFF
	*/
	priv_data->wp_gpio = zmp_mmc_setup_gpio_in(wp_gpio, "mmc_wp"); 
	if (priv_data->wp_gpio != -1) {
		debug("yes wp_gpio=%d\n", priv_data->wp_gpio);
		mmc->getwp = zmp_mmc_getwp; 
	}
	...
	*/
	mmc->b_max    = SD_DMA_BLOCK_4K; //  support for multi block max cnts
...
bool sd_init_info(int idex, struct mmc_cd_info *mmc_cd)
{
	...
	//rt_kprintf("g_compatible[:%d]: %s\n",idex, g_compatible[idex]);
	//node = fdtdec_get_node_by_compatible((const void *)s_fdt_dtb, 0, g_compatible[idex]);
	node = fdtdec_next_compatible(s_fdt_dtb, 0, COMPAT_ZMP_H3D_MCI0+idex);
	if(node < 0)
	{
		printf("fdt err:%d\n", ret);
		return false;
