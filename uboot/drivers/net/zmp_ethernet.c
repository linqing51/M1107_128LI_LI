...
#define NO_AUTOPROBE
#define SMC_DEBUG 0
#define ZMP_VA_L2CTRL (0x20140000)
...
#define SMC_DEV_NAME "Ethernet"
#define SMC_PHY_ADDR 0x0005
...
#define ZMP_PA_GPIOCTRL		(0x20170000)
...
#define DEFAULT_PHY_BASE PHY5 //We use First Phy 
...
typedef struct synopGMACDeviceStruct
{
	...
	DmaDesc *rx_ring[RECEIVE_DESC_SIZE]; // add
	DmaDesc *tx_ring[TRANSMIT_DESC_SIZE]; // add
	...
	u32	rx_tmp_len;			/* Gmac ctrl interrupt bit  */
} gmac_device;
...
enum Mii_Phy_Status
{
    ...
    Mii_phy_status_full_duplex	= 0x0100, // bit 8, old,13
...
enum GmacGmiiAddrReg
{
    ...
    GmiiCsrClkMask	   = 0x0000003C,     /* CSR Clock bit Mask			 4:2	, must :0x0000001C		     */
...
typedef enum {
	ZMP_PULLDOWN_ENABLE = 0,
	ZMP_PULLDOWN_DISABLE
} T_GPIO_PD_CONFG;

struct zmp_ethernet_priv{
	u8 dev_num;
};
...
static s32 gmac_reset(gmac_device *gmacdev)
{
    ...
    gmac_write_reg((u32 *)gmacdev->DmaBase, DmaBusMode , DmaResetOn); // reset dma engine
...
static s32 gmac_set_mdc_clk_div(gmac_device *gmacdev, u32 clk_div_val)
{
    ...
    orig_data &= (~ GmiiCsrClkMask); // csr clk bit[5:2], must 0x3C
...
static u32 synopGMAC_get_mdc_clk_div(gmac_device *gmacdev)
{
    ...
    // debug("mdc_clk_div1:0x%x\n", data);
    data &= GmiiCsrClkMask;
    // debug("mdc_clk_div2:0x%x\n", data);
    return data;
}
...
unsigned int get_eth_reset_gpio(void)
{
	...
	node = fdtdec_next_compatible(s_fdt_dtb, 0, COMPAT_ZMP_H3D_ETH);
	if(node < 0)
	{
...
static s32 gmac_phy_init(gmac_device *gmacdev)
{
	...
	if(0x1 == phy_wait_idle(gmacdev)) {
		printf("%s,phy_wait_idle err!\n", __func__);
	}else {
		debug("%s,phy_wait_idle ok!\n", __func__);
	}
	...
	if(data_id1 == 0x22) {
		...
		data_ctl |= 0x1000; // enable auto-negotiation process
		gmac_write_phy_reg((u32 *)gmacdev->MacBase, gmacdev->PhyBase, PHY_CONTROL_REG, data_ctl);
	}else {
		debug("id other set!\n");
		gmac_read_phy_reg((u32 *)gmacdev->MacBase, gmacdev->PhyBase, PHY_CONTROL_REG, &data_ctl);
		data_ctl |= 0x1100; // bit12 enable auto-negotiation process, set full-duplex mode
		gmac_write_phy_reg((u32 *)gmacdev->MacBase, gmacdev->PhyBase, PHY_CONTROL_REG, data_ctl);
...
//    loop_count = 10; 
	...
	/* note may be for same special phy define register, not same 8201F phy, PHY_SPECIFIC_STATUS_REG */ 
    status = gmac_read_phy_reg((u32 *)gmacdev->MacBase, gmacdev->PhyBase, PHY_STATUS_REG, &data);
    if(status) {
        return status;
    }

    if((data & Mii_Link) == 0) { // Mii_phy_status_link_up
        printf("No Link\n");
...
{
	desc->status = 0; 
    desc->length = last_ring_desc ? TxDescEndOfRing : 0; // TxDescEndOfRing bit25(TER)
    desc->buffer1 = 0;
    desc->buffer2 = 0;
    desc->data1 = 0;
    desc->data2 = 0;
...
static s32 setup_tx_desc_queue(gmac_device *gmacdev, struct eth_device *pnetdev, u32 no_of_desc, u32 desc_mode)
{
    ...
    if(desc_mode == RINGMODE) {
        ...
        gmacdev->TxDescCount = no_of_desc;	// ring descriptor count
        gmacdev->tx_ring[0]  = first_desc;	// ring descriptor base virtual address
        gmacdev->TxDescDma   = dma_addr;	// ring descriptor base physic address
...
static s32 setup_rx_desc_queue(gmac_device *gmacdev, struct eth_device *pnetdev, u32 no_of_desc, u32 desc_mode)
{
    ...
    if(desc_mode == RINGMODE) {
        ...
        gmacdev->rx_ring[0]  = first_desc; // the first desc virtual addr
        gmacdev->RxDescDma   = dma_addr;   // the first desc physic addr
...
static s32 gmac_mac_init(gmac_device *gmacdev)
{
    ...
    if(gmacdev->DuplexMode == FULLDUPLEX)
    {
        ...
        gmac_tx_enable(gmacdev); // TX Enable
        gmac_rx_enable(gmacdev); // RX Enable

        if(gmacdev->Speed == SPEED1000)
            gmac_select_gmii(gmacdev);
        else
            gmac_select_mii(gmacdev); // we use mii
...
static int smc_init(struct eth_device *dev, bd_t *bd)
{
	...
	/* software reset , the resets all of the GMAC internal registers and logic, check ok */ 
	...
	*If these are not disabled ISR should be modified accordingly to handle these interrupts.
	 */
	gmac_disable_mmc_tx_interrupt(gmacdev, 0xFFFFFFFF); // set 1for mask interrupt
...
/**
  * @brief enable or disable gpio49 pulldown
  *
  ...
  * @return void.
  */
static void zmp_gpio_disable_pupd(T_GPIO_PD_CONFG gpio_pddis_en)
{
	/* set chip system ctrl reg base address, 1:disable, 0:enable */ 
	if (gpio_pddis_en){
		REG32(ZMP_PA_SYSCTRL + 0x194) |= (0x1<<18); // disable gpio49 pull down
	}else {
		REG32(ZMP_PA_SYSCTRL + 0x194) &= ~(0x1<<18); // enable gpio49 pull down
	}
}
...
static void gmac_rmii_interface_50mclk(void)
{
	...
	*/
	...
	REG32(ZMP_PA_SYSCTRL + 0x14) &=  ~(0x1 << 22); // first mac interface select Rmii
	REG32(ZMP_PA_SYSCTRL + 0x14) |=  (0x1 << 23); // first  mac_speed_cfg=1(100m)

	REG32(ZMP_PA_SYSCTRL + 0x14) |=  (0x1 << 21); // bit[21],enable generate 50m
	REG32(ZMP_PA_SYSCTRL + 0x14) |=  ((0x1 << 28)|(0x1 << 18)); // set   bit[21],enable generate 50m,	bit[18], select 25m clock of mac from pll div
	REG32(ZMP_PA_SYSCTRL + 0x1c) &= ~(1 << 13);		 // mac ctronller clk 
}
...
static void gmac_clk_sharepin_init(void)
{
	...
	zmp_gpio_disable_pupd(ZMP_PULLDOWN_DISABLE);
...
/**
  * @brief attach mac hardware address
  *
...
/**
* @brief  ethernet device driver initialize
*
*  initialize and register ethernet device driver 
* @author CaoDonghua
* @date 2016-09-26
...
*/
int zmp_ethernet_initialize(u8 dev_num, int base_addr)
{
	...
	*/
	...
	dev->iobase = base_addr; // 0x20300000
	dev->init = smc_init;
	dev->halt = smc_halt;
	dev->send = smc_send;
	dev->recv = smc_rcv;
