/*
 * drivers/watchdog/zmp_top_wdt.c
 * Watchdog driver for zmp110x processors
 *
...
#define MODULE_WDT_CFG1	(ZMP_VA_SYSCTRL + 0xe4)
#define MODULE_WDT_CFG2	(ZMP_VA_SYSCTRL + 0Xe8)

static unsigned long in_use;
static atomic_t in_write = ATOMIC_INIT(0);
static DEFINE_SPINLOCK(wdt_lock);

static int zmp_wdt_disable_nb(struct notifier_block *n, unsigned long state,void *cmd);
static struct notifier_block zmp_wdt_nb = {
	.notifier_call = zmp_wdt_disable_nb,
};
...
static struct watchdog_info ident = {
	...
	.identity	= "ZLGMCU zmp110x Watchdog",
};

static long zmp_wdt_ioctl(struct file *file, unsigned int cmd,
							unsigned long arg)
{
...
static ssize_t zmp_wdt_write(struct file *file, const char *data, size_t len, loff_t *ppos)
{
	if (len) {
		size_t i;
...
static int zmp_wdt_open(struct inode *inode, struct file *file)
{
...
static int zmp_wdt_release(struct inode *inode, struct file *file)
{
	if (nowayout)
...
static const struct file_operations zmp_wdt_fops = {
	.owner		= THIS_MODULE,
	.llseek		= no_llseek,
	.write		= zmp_wdt_write,
	.unlocked_ioctl	= zmp_wdt_ioctl,
	.open		= zmp_wdt_open,
	.release	= zmp_wdt_release,
};

static struct miscdevice zmp_wdt_miscdev = {
	.minor		= WATCHDOG_MINOR,
	.name		= "watchdog",
	.fops		= &zmp_wdt_fops,
};

static int zmp_watchdog_probe(struct platform_device *pdev)
{

	int ret = 0;
	
    pr_info("zmp_wdt_init: watchdog register...\n");
	register_reboot_notifier(&zmp_wdt_nb);
	...
	ret = misc_register(&zmp_wdt_miscdev);
	if(ret) {
...
static int zmp_watchdog_remove(struct platform_device *pdev)
{
	unregister_reboot_notifier(&zmp_wdt_nb);

	misc_deregister(&zmp_wdt_miscdev);

	return 0;
}

static const struct of_device_id zmp_watchdog_of_ids[] = {
    { .compatible = "zlgmcu,zmp110x-wdt" },
	{},
};
MODULE_DEVICE_TABLE(of, zmp_watchdog_of_ids);

static struct platform_driver zmp_watchdog_driver = {
	.driver		= {
		.name	= "zmp-watchdog",
        .of_match_table = of_match_ptr(zmp_watchdog_of_ids),
		.owner	= THIS_MODULE,
	},
	.probe		= zmp_watchdog_probe,
	.remove		= zmp_watchdog_remove,
};

static int __init zmp_wdt_init(void)
{	
	return platform_driver_register(&zmp_watchdog_driver);
}

static void __exit zmp_wdt_exit(void)
{
	platform_driver_unregister(&zmp_watchdog_driver);
}
module_init(zmp_wdt_init);
module_exit(zmp_wdt_exit);

MODULE_DESCRIPTION("ZLGMCU zmp110x Watchdog driver");
MODULE_LICENSE("GPL");
