...
static int zmp_scan_block_fast(struct mtd_info *mtd, struct nand_bbt_descr *bd,
...
static inline struct zmp_spinand *mtd_to_spiflash(struct mtd_info *mtd)
{
	return container_of(mtd, struct zmp_spinand, mtd);
}
...
*/
static void *flash_buf_bounce_pre(struct zmp_spinand *flash,
...
* @retval return the transition buffer
*/
static void flash_buf_bounce_post(struct zmp_spinand *flash,
				void *buf, u32 len, int dir)
{
	if(!is_vmalloc_addr(buf)) {
		return;
...
#else
static inline void *flash_buf_bounce_pre(struct zmp_spinand *flash,
				void *buf, u32 len, int dir)
{
	return buf;
}

static inline void flash_buf_bounce_post(struct zmp_spinand *flash,
				void *buf, u32 len, int dir)
{
}
#endif
...
*/
static int read_sr(struct zmp_spinand *flash, u32 addr)
{
...
*/
static int write_sr(struct zmp_spinand *flash, u32 addr, u16 val)
{
...
static inline int sflash_reset(struct zmp_spinand *flash)
{
...
* @retval return a negative error code if failed
*/
static inline int write_enable(struct zmp_spinand *flash)
{
...
* @retval return a negative error code if failed
*/
static inline int write_disable(struct zmp_spinand *flash)
{
	u8	code = OPCODE_WRDI;
...
*/
static int wait_till_ready(struct zmp_spinand *flash)
{
...
static int check_ecc_status(struct zmp_spinand *flash)
{
	unsigned long deadline;
	int idx, shift;
	u32 sr;
...
* @param[in] flash  spiflash handle.
*/
static int quad_mode_enable(struct zmp_spinand *flash)
{
	int ret, idx, shift;
	u32 regval;
	u8 addr;
...
* @param[in] flash  spiflash handle.
*/
static int quad_mode_disable(struct zmp_spinand *flash)
{
	int ret, idx, shift;
	u32 regval;
	u8 addr;
...
* @retval return a non-zero error code if failed
*/
static int erase_block(struct zmp_spinand *flash, u32 offset)
{
	u32 row;
	struct spi_transfer t[1];
	struct spi_message m;
...
* @retval return a non-zero error code if failed
*/
static int zmp_spinand_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	struct zmp_spinand *flash = mtd_to_spiflash(mtd);
	u32 addr,len;
	uint32_t rem;
	...
	{
		printk(KERN_ERR "zmp_spinand_erase:instr->addr[0x%llx] + instr->len[%lld] > mtd->size[%lld]\n",
	...
	if (rem != 0)
	{
		printk(KERN_ERR "zmp_spinand_erase:rem!=0 [%u]\n", rem );
		return -EINVAL;
	}
...
static int init_spiflash_rw_info(struct zmp_spinand *flash)
{
	/**default param.*/
...
* @retval return a non-zero error code if failed
*/
static int zmp_spinand_cfg_quad_mode(struct zmp_spinand *flash)
{
	int ret = 0;
	if((flash->bus_width & FLASH_BUS_WIDTH_4WIRE) &&
...
* @retval return a non-zero error code if failed
*/
static int zmp_spinand_read(struct mtd_info *mtd, loff_t from, size_t len,
	...
	size_t *retlen, const u_char *buf)
{
	...
	*retlen = m.actual_length - CMD_ADDR_SIZE;
	...
	PDEBUG("zmp_spinand_write: retlen=%ld\n", *retlen);
...
static int zmp_spinand_write(struct mtd_info *mtd, loff_t to, size_t len,
		size_t *retlen, const u_char *buf)
{
	int ret = 0;
	size_t rlen = 0;
...
*/
static int adjust_xfer_len(struct zmp_spinand *flash, int column, int readlen)
{
	int seg_oob;
	int xfer_len;
...
* @retval return a non-zero error code if failed
*/
static int spinand_oobpos_to_coord(struct zmp_spinand *flash,
...
static int zmp_spinand_read_oob(struct mtd_info *mtd, loff_t from,
			 struct mtd_oob_ops *ops)
{
	int ret = -ENOTSUPP;
...
static int zmp_spinand_write_oob(struct mtd_info *mtd, loff_t to,
			 struct mtd_oob_ops *ops)
{
	int ret = -ENOTSUPP;
...
static int _zmp_spinand_read(struct mtd_info *mtd, loff_t from, size_t len,
			size_t *retlen, u_char *buf)
{
	pr_debug("%s  offs:0x%llx\n",__func__,(long long)from);
	zmp_spinand_read(mtd,from,len,retlen,buf);
	return 0;
}

static int _zmp_spinand_write(struct mtd_info *mtd, loff_t to, size_t len,
		size_t *retlen, const u_char *buf)
{
	pr_debug("%s  offs:0x%llx\n",__func__,(long long)to);
	zmp_spinand_write(mtd,to,len,retlen,buf);
	return 0;
}

/**
 * zmp_nand_default_block_markbad - [DEFAULT] mark a block bad via bad block marker
 ...
 * block.
 */
static int zmp_nand_default_block_markbad(struct mtd_info *mtd, loff_t ofs)
{
...
static inline void zmp_bbt_mark_entry(struct nand_chip *chip, int block,
		uint8_t mark)
{
...
/**
 * zmp_nand_markbad_bbt - [NAND Interface] Mark a block bad in the BBT
 * @mtd: MTD device structure
 * @offs: offset of the bad block
 */
static int zmp_nand_markbad_bbt(struct mtd_info *mtd, loff_t offs)
{
	...
	zmp_bbt_mark_entry(this, block, BBT_BLOCK_WORN);   /* if generate bad block durning operate(erase,write),mark 0x01 */

	return ret;
}

/**
 * zmp_nand_block_markbad_lowlevel - mark a block bad
 * @mtd: MTD device structure
 * @ofs: offset from device start
 *
...
*/
static int zmp_nand_block_markbad_lowlevel(struct mtd_info *mtd, loff_t ofs)
{
	struct nand_chip *chip = mtd->priv;
	int res, ret = 0;
	...
	if (!(chip->bbt_options & NAND_BBT_NO_OOB_BBM)) {
		...
		zmp_spinand_erase(mtd, &einfo);   /* erase block */
		#endif

		/* Write bad block marker to OOB */
		ret = zmp_nand_default_block_markbad(mtd, ofs);
	...
	if (chip->bbt) {
		res = zmp_nand_markbad_bbt(mtd, ofs);
		if (!ret)
			ret = res;
...
static inline uint8_t zmp_bbt_get_entry(struct nand_chip *chip, int block)
{
...
/**
 * zmp_nand_isbad_bbt - [NAND Interface] Check if a block is bad
 * @mtd: MTD device structure
 * @offs: offset in the device
 * @allowbbt: allow access to bad block table region
 */
static int zmp_nand_isbad_bbt(struct mtd_info *mtd, loff_t offs, int allowbbt)
{
	struct nand_chip *this = mtd->priv;
	int block, res;

	block = (int)(offs >> this->bbt_erase_shift);
	res = zmp_bbt_get_entry(this, block);   /* 2bit per block, find the block from bbt */

	pr_debug("nand_isbad_bbt(): bbt info for offs 0x%08x: (block %d) 0x%02x\n",
		 (unsigned int)offs, block, res);

	if(res)
		printk("%s:block %d is a badblock", __func__, block);
...
/**
 * zmp_nand_block_checkbad - [GENERIC] Check if a block is marked bad
 * @mtd: MTD device structure
 * @ofs: offset from device start
 ...
 */
static int zmp_nand_block_checkbad(struct mtd_info *mtd, loff_t ofs, int getchip,
			       int allowbbt)
{
	struct nand_chip *chip = mtd->priv;
	int numpages = 0;
	...
	if (!chip->bbt)
	{
		...
		return zmp_scan_block_fast(mtd, chip->badblock_pattern, from, chip->buffers->databuf, numpages);
		//return -EINVAL;
	}

	/* Return info from the table */
	return zmp_nand_isbad_bbt(mtd, ofs, allowbbt);
}

/**
 * zmp_new_spinand_block_isbad - Read bad block marker in bbt
 * @mtd:	MTD device structure
 * @ofs:	offset from device start
 *
 * Check, if the block is bad.
 */
int zmp_new_spinand_block_isbad(struct mtd_info *mtd, loff_t offs)
{
	pr_debug("%s  offs:0x%llx\n",__func__,(long long)offs);
	return zmp_nand_block_checkbad(mtd, offs, 1, 0);
}

int zmp_new_spinand_block_markbad(struct mtd_info *mtd, loff_t offs)
{
	int ret;
	pr_debug("%s  offs:0x%llx\n",__func__,(long long)offs);

	ret = zmp_new_spinand_block_isbad(mtd, offs);   /* check badblock before markblock */
	if (ret) {
	...
	return zmp_nand_block_markbad_lowlevel(mtd, offs);
}
...
*/
static int zmp_spinand_get_devid(struct mtd_info *mtd)
{
	struct zmp_spinand *flash = mtd_to_spiflash(mtd);
	int			ret;
	u8			id[5];
	...
	if (ret < 0) {
		DEBUG(MTD_DEBUG_LEVEL0, "%s: error %d zmp_spinand_get_devid\n",
...
static int zmp_spinand_init_stat_reg(struct zmp_spinand *flash)
{
...
int zmp_set_chip(struct nand_chip *chip, struct flash_info *info)
{
...
static int zmp_mtd_read_badflag(struct mtd_info *mtd, loff_t from, struct mtd_oob_ops *ops)
{
	int ret_code;
...
/**
 * zmp_check_short_pattern - [GENERIC] check if a pattern is in the buffer
 ...
 * check.
 */
static int zmp_check_short_pattern(uint8_t *buf, struct nand_bbt_descr *td)
{
	/* Compare the pattern */
			   ...
			   loff_t offs, uint8_t *buf, int numpages)
{
	...
	for (j = 0; j < numpages; j++) {
		 ...
		 * read oob data
		 */
		ret = zmp_mtd_read_badflag(mtd, offs, &ops);
		...
		if (zmp_check_short_pattern(buf, bd))
		{
			printk("%s:block %lld is a badblock", __func__, offs >> (ffs(mtd->erasesize) - 1));
			return 1;
		}
...
/**
 * zmp_create_bbt - [GENERIC] Create a bad block table by scanning the device
 * @mtd: MTD device structure
 ...
 * identify pattern.
 */
static int zmp_create_bbt(struct mtd_info *mtd, uint8_t *buf,
	struct nand_bbt_descr *bd, int chip)
{
	...
	if (chip == -1) {
		...
		if (chip >= this->numchips) {
			pr_warn("zmp_create_bbt(): chipnr (%d) > available chips (%d)\n",
	...
	for (i = startblock; i < numblocks; i++) {
		...
		ret = zmp_scan_block_fast(mtd, bd, from, buf, numpages);   /* check the block is good or not */
		//ret = 0;
		...
		if (ret) {
		    zmp_bbt_mark_entry(this, i, BBT_BLOCK_FACTORY_BAD);   /* factory bad block mark 0x3 in bbt */
...
/**
 * zmp_nand_memory_bbt - [GENERIC] create a memory based bad block table
 * @mtd: MTD device structure
 * @bd: descriptor for the good/bad block search pattern
 *
 ...
 */
static inline int zmp_nand_memory_bbt(struct mtd_info *mtd, struct nand_bbt_descr *bd)
{
	struct nand_chip *this = mtd->priv;

	return zmp_create_bbt(mtd, this->buffers->databuf, bd, -1);   /* create bbt */
}

/**
 * zmp_nand_default_bbt - [NAND Interface] Select a default bad block table for the device
 * @mtd: MTD device structure
 *
 * This function selects the default bad block table support for the device and
 * calls the nand_scan_bbt function.
 */
int zmp_nand_default_bbt(struct mtd_info *mtd, struct nand_bbt_descr *bd)
{
	struct nand_chip *this = mtd->priv;
	int len, res;
	...
	if (!td) {
		...
		if ((res = zmp_nand_memory_bbt(mtd, bd))) {
...
/**
 * zmp_spinand_enable_ecc- send command 0x1f to write the SPI Nand OTP register
 * Description:
 *   There is one bit( bit 0x10 ) to set or to clear the internal ECC.
 *   Enable chip internal ECC, set the bit to 1
 *   Disable chip internal ECC, clear the bit to 0
 */
static int zmp_spinand_enable_ecc(struct zmp_spinand *flash)
{
	int retval;
	u8 otp = 0;
...
static int zmp_spinand_disable_ecc(struct zmp_spinand *flash)
{
	int retval;
	u8 otp = 0;
...
int spinand_read_partition(unsigned long start_pos, unsigned long partition_size, unsigned char *buf, unsigned long buf_len)
{
	...
	unsigned long page_size = zmp_mtd_info->writesize;
	unsigned long erase_size = zmp_mtd_info->erasesize;
	loff_t offs = 0;
	size_t retlen = 0;

	//pr_err("start_pos:%d, partition_size:%d, buf_len:%d\n", start_pos, partition_size, buf_len);
	while(1)
	{
		...
		if(zmp_new_spinand_block_isbad(zmp_mtd_info, offs) == 1)
		{
...
*/
//static
int zmp_spinand_probe(struct spi_device *spi)
{

	unsigned	i,bus_width = 0;
	unsigned	ret = 0;
	struct flash_info						*info;
	struct zmp_spinand						*flash;
	...
	ret = zmp_set_chip(chip,info);    /* use flash_info to setup nand_chip */
	if (ret != 0)
		return ret;
	...
	flash->mtd.writebufsize  = flash->info.page_size;
	flash->mtd.flags = MTD_WRITEABLE;
	flash->mtd.size = info->block_size * info->n_blocks;
	flash->mtd._erase = zmp_spinand_erase;
	flash->mtd._write = _zmp_spinand_write;
	flash->mtd._read = _zmp_spinand_read;
	flash->mtd._read_oob = zmp_spinand_read_oob;
	flash->mtd._write_oob = zmp_spinand_write_oob;

	flash->mtd._block_isbad = zmp_new_spinand_block_isbad;
	flash->mtd._block_markbad = zmp_new_spinand_block_markbad;
	flash->mtd.get_device_id = zmp_spinand_get_devid;
	...
	printk("flash->bus_width:%d(note:1:1 wire, 2:2 wire, 4:4 wire)\n", flash->bus_width);
	...
	zmp_spinand_init_stat_reg(flash);
	zmp_spinand_cfg_quad_mode(flash);
	...
	zmp_spinand_disable_ecc(flash);
	ret = zmp_nand_default_bbt(&flash->mtd, chip->badblock_pattern);
	zmp_spinand_enable_ecc(flash);
	if(ret)
	{
		printk("scan bbt failed in memory\n");
		kfree(flash->buf);
		kfree(flash);
		return ret;
	}
...
* @retval return a non-zero error code if failed
*/
static int zmp_spinand_remove(struct spi_device *spi)
{
	struct zmp_spinand	*flash = dev_get_drvdata(&spi->dev);
	int		status;
...
static struct spi_driver zmp_spinand_driver = {
	.driver = {
		.name	= "zmp-spinand",
		.bus	= &spi_bus_type,
		.owner	= THIS_MODULE,
	...
	.probe	= zmp_spinand_probe,
	.remove	= zmp_spinand_remove,
...
* @retval return a non-zero error code if failed
*/
static int __init zmp_spinand_init(void)
{
	return spi_register_driver(&zmp_spinand_driver);
}
...
* @return None
*/
static void __exit zmp_spinand_exit(void)
{
	spi_unregister_driver(&zmp_spinand_driver);
}


module_init(zmp_spinand_init);
module_exit(zmp_spinand_exit);

MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("MTD SPI Nand flash driver for ZMP chips");
