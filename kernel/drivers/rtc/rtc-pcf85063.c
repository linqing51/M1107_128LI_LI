...
#define CAP_SEL_0	_IO('p', 0x22)
#define CAP_SEL_1	_IO('p', 0x23)
#define CAP_OFFSET	_IO('p', 0x24)
								...
								unsigned long arg)
{
	...
	case CAP_SEL_1: /*12.5 pF*/
		...
		break;
	case CAP_OFFSET: /* config offset */
		val = arg;
		buf[0] = PCF85063_REG_OFFSET;
		buf[1] = val;
		err = i2c_master_send(client, buf, sizeof(buf));
		if(err != sizeof(buf)) {
			dev_err(&client->dev, \
				...
				const struct i2c_device_id *id)
{
	...
	pcf85063_rtc_ioctl(&client->dev, CAP_SEL_1, 0);
	pcf85063_rtc_ioctl(&client->dev, CAP_OFFSET, 0x08);
