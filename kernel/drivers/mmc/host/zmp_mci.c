/*
 *  linux/drivers/mmc/host/zmp-mci.c - ZMP110x MMC/SD/SDIO driver
 *
 *
...
#define DRIVER_NAME 		"zmpmci"

#define ___hdbg___()		printk("zmpmci:----func:%s---line:%d----\n", __func__, __LINE__);
...
static void zmpmci_l2xfer(struct zmpmci_host *host);
static void zmpmci_init_host_cfg(struct zmpmci_host *host);
 ...
 * @return void.
*/
static inline void dbg_dumpregs(struct zmpmci_host *host,
...
#define ZMP_PPU_PPD_S1           (ZMP_VA_SYSCTRL + 0x1A4)
#define ZMP_PPU_PPD_S2     		(ZMP_VA_SYSCTRL + 0x1E8)
#define ZMP_PPU_PPD_S3           (ZMP_VA_SYSCTRL + 0x19C)
#define ZMP_PPU_PPD_IE           (ZMP_VA_SYSCTRL + 0x1F8)

#define ZMP_PPU_PPD1           	(ZMP_VA_SYSCTRL + 0x194)
#define ZMP_PPU_PPD2           	(ZMP_VA_SYSCTRL + 0x198)
#define ZMP_PPU_PPD3           	(ZMP_VA_SYSCTRL + 0x1A0)

	dev_dbg(&pdev->dev,"ZMP_SHAREPIN_CON0:[%08x], ZMP_SHAREPIN_CON1:[%08x], ZMP_SHAREPIN_CON2:[%08x], ZMP_SHAREPIN_CON3:[%08x], ZMP_SHAREPIN_CON4:[%08x],ZMP_SHAREPIN_CON5:[%08x], ZMP_SHAREPIN_CON6:[%08x]\n",
		readl(ZMP_SHAREPIN_CON0), readl(ZMP_SHAREPIN_CON1), 
		readl(ZMP_SHAREPIN_CON2), readl(ZMP_SHAREPIN_CON3),
		readl(ZMP_SHAREPIN_CON4), readl(ZMP_SHAREPIN_CON5),readl(ZMP_SHAREPIN_CON6));

	dev_dbg(&pdev->dev,"ZMP_PPU_PPD1:[%08x], ZMP_PPU_PPD2:[%08x], ZMP_PPU_PPD3:[%08x], ZMP_PPU_PPD_S1:[%08x], ZMP_PPU_PPD_S2:[%08x],ZMP_PPU_PPD_S3:[%08x], ZMP_PPU_PPD_IE:[%08x]\n",
		readl(ZMP_PPU_PPD1), readl(ZMP_PPU_PPD2),readl(ZMP_PPU_PPD3), 
		readl(ZMP_PPU_PPD_S1),readl(ZMP_PPU_PPD_S2),readl(ZMP_PPU_PPD_S3),
		readl(ZMP_PPU_PPD_IE));	
 ...
 * @return void.
*/
static inline void dbg_dumpdata(struct zmpmci_host *host,
		void *data, int size)
{
	int ii;
	int dsize = (size +3)/4;
...
/**
* zmpmci_xfer_mode - judgement the mci transfer mode.
* ret: 	ZMPMCI_XFER_L2DMA: use for l2 dma mode
*/
static inline int zmpmci_xfer_mode(struct zmpmci_host *host)
{
	return host->xfer_mode;
}

static void  config_plugoff_int(struct zmpmci_host *host, int enable)
{
	u32 val;
	unsigned long flags;
...
static void  config_plugin_int(struct zmpmci_host *host, int enable)
{
	u32 val;
	unsigned long flags;
	/*lock for write mci_pluginoff reg0*/
 ...
 */
int _get_card_status(struct zmpmci_host *host)
{
	struct platform_device	*pdev = host->pdev;
	...
	while (1) {
			...
			for (i = 0; i < 3; i++) {
				if(host->detect_mode == ZMPMCI_DETECT_MODE_CLK)
				{
					spin_lock_irqsave(&host->lock, flags);
					...
					if(mci_active)
					{
						dev_info(&pdev->dev, "Data transmission is in progress. MCI[2]_CLK will be pulled down, and can not function as the detection pin.!");
						return CARD_PLUGED;  // "Data transmission is in progress" treat as CARD_PLUGED(1)
					}
					status[i] = mci_status;
					spin_unlock_irqrestore(&host->lock, flags);
				}
				else if(host->detect_mode == ZMPMCI_DETECT_MODE_GPIO)
				{
...
int get_card_status(struct zmpmci_host *host)
{
	struct platform_device	*pdev = host->pdev;
	unsigned int status = CARD_PLUGED;
	__maybe_unused unsigned int status_reg;

	if(host->detect_mode != ZMPMCI_PLUGIN_ALWAY)
...
/* prenset = 1 */
static int zmpmci_card_present(struct mmc_host *mmc)
{
	struct zmpmci_host *host = mmc_priv(mmc);
	...
	pr_debug("zmpmci_card_present, card_pwr_en=%d, card_status=%d\n",card_pwr_en,host->card_status );
	
	return (host->card_status && card_pwr_en);
}

static inline int enable_imask(struct zmpmci_host *host, u32 imask)
{
	u32 newmask;
...
static inline int disable_imask(struct zmpmci_host *host, u32 imask)
{
	u32 newmask;
...
static inline void clear_imask(struct zmpmci_host *host)
{
...
{	
	struct zmpmci_host *host = dev_id;
	struct platform_device	*pdev = host->pdev;
	int curr_status = 0;	
	int ret;
...
{
	struct zmpmci_host *host = (struct zmpmci_host *)data;
	struct platform_device	*pdev = host->pdev;
...
static void zmpmci_l2xfer(struct zmpmci_host *host)
{
	...
	if(xferlen==0){
		dev_dbg(&pdev->dev, "##zmpmci_l2xfer %s xferlen==0\n", (host->data->flags & MMC_DATA_WRITE)?"write":"read");
		return;		
	}
	if (USED_DMA(xferlen))		
	//if ((zmpmci_xfer_mode(host) == ZMPMCI_XFER_L2DMA) && USED_DMA(xferlen))	
	{
		...
		if(sg_num>1){
			...
			l2_combuf_dma(phyaddr, host->l2buf_id, xferlen, dir, ZMP_TRUE);   
		}else {
			dev_dbg(&pdev->dev, "##sg_num==1\n");
			l2_combuf_dma(phyaddr, host->l2buf_id, xferlen, dir, ZMP_FALSE);	 
		}
	} 
	else if(USED_DMA_CPU(xferlen)) 
	//else if((zmpmci_xfer_mode(host) == ZMPMCI_XFER_L2DMA) && USED_DMA_CPU(xferlen)) 
	{
		unsigned int need_callback=0;
		dev_dbg(&pdev->dev, "##zmpmci transfer data: DMA AND CPU mode.\n");
		phyaddr = sg_dma_address(host->sg_ptr) + host->sg_off;
	...
	else 
	{
		...
		dev_dbg(&pdev->dev,"##zmpmci transfer data: CPU mode.\n");
...
void zmpmci_init_sg(struct zmpmci_host *host, struct mmc_data *data)
{
	/*
...
int zmpmci_next_sg(struct zmpmci_host *host)
{
	host->sg_ptr++;
	host->sg_off = 0;
	return --host->sg_len;
 ...
 * @return void.
 */
static void zmpmci_stop_data(struct zmpmci_host *host)
{
	//struct platform_device	*pdev = host->pdev;	
	...
	if(zmpmci_xfer_mode(host) ==ZMPMCI_XFER_L2DMA) {
		if (host->data->flags & MMC_DATA_WRITE) {
 ...
 * @return void.
 */
static void zmpmci_request_end(struct zmpmci_host *host, struct mmc_request *mrq)
{
	struct platform_device	*pdev = host->pdev;
	...
	dev_dbg(&pdev->dev, "zmpmci_request_end. buf_id=%d\n", host->l2buf_id);
	...
	dev_dbg(&pdev->dev, "finalize the mci request.\n");
	 ...
	 /*if request fail,then mmc_request_done send request again, zmpmci_send_request
 ...
 * @return void.
 */
static void zmpmci_start_command(struct zmpmci_host *host, struct mmc_command *cmd)
{
	struct platform_device	*pdev = host->pdev;
	unsigned int ccon;
	...
	ccon |= 1<<10;//rspcrcnochk 1 = CPSM doesn’t check the CRC of the response.
...
static void print_mci_data_err(struct mmc_data *data,unsigned int status, const char *err)
{
	if (data->flags & MMC_DATA_READ) {
		pr_debug("zmpmci: data(read) status=0x%x data_blocks=%d data_blocksize=%d %s\n", status, data->blocks, data->blksz, err);
	} else if (data->flags & MMC_DATA_WRITE) {
		pr_debug("zmpmci: data(write) status=0x%x data_blocks=%d data_blocksize=%d %s\n", status, data->blocks, data->blksz, err);
	}
}
 ...
 */
 static irqreturn_t zmpmci_irq(int irq, void *dev_id)
{
	struct zmpmci_host *host = dev_id;
	...
	dev_dbg(&pdev->dev, "###zmpmci_irq status : %x\n", status);
...
static int zmpmci_setup_data(struct zmpmci_host *host, struct mmc_data *data)
{
	...
	if (host->size > 64 * 1024)
		dev_err(&pdev->dev, "Err: %s %d zmpmci %s to long: %d.\n", __func__, __LINE__,
				(data->flags & MMC_DATA_WRITE) ? "write":"read", host->size);

	zmpmci_init_sg(host, data); 
...
static int zmpmci_wait_cmd_complete(struct zmpmci_host *host)
{
	int ret = 0;
	unsigned int status;
	...
	while(1){
		...
		do {
			...
			if (test_bit(MCI_CMD_IRQ, &host->pending_events))
			{
				...
				dev_dbg(&pdev->dev, "end zmpmci_wait_cmd_complete ret=%d\n", ret);
...
static int zmpmci_wait_data_complete(struct zmpmci_host *host)
{
	unsigned int status;
	...
	{
		//if((zmpmci_xfer_mode(host) == ZMPMCI_XFER_L2DMA) && 
		if((ZMP_TRUE == !l2_combuf_wait_dma_finish(host->l2buf_id)))
		{
			clear_bit(MCI_DATA_IRQ, &host->pending_events);
			return 0;
		}
...
static void zmpmci_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	struct zmpmci_host *host = mmc_priv(mmc);
	struct platform_device	*pdev = host->pdev;
	int ret;
	__maybe_unused unsigned int clk;
	...
	if(mrq->data){
		ret = zmpmci_setup_data(host, mrq->data);
		if (ret) {
			mrq->data->error = ret;
	...
	zmpmci_start_command(host, mrq->cmd);
	ret = zmpmci_wait_cmd_complete(host);
	if (ret) {
		if(mrq->data)
			zmpmci_stop_data(host);
		goto zmpmci_req_end;
	}
	...
	if(mrq->data){
		if(mrq->data->flags & MMC_DATA_READ){
			...
			ret = zmpmci_wait_data_complete(host);
			if(ret){
				//dev_err(&pdev->dev,"MMC_DATA_READ err = %d\n", ret);
				goto zmpmci_data_err;
			}
		...
		if (mrq->stop){
			...
			zmpmci_start_command(host, mrq->stop);
			ret = zmpmci_wait_cmd_complete(host);
			if (ret) {
 ...
 */
static void zmpmci_set_clk(struct zmpmci_host *host, struct mmc_ios *ios)
{
	...
	if (ios->clock == 0) {
		...
		dev_dbg(&pdev->dev, "gclk = %lu Hz. div_l=%u, div_h=%u\n",host->gclk, clk_div_l, clk_div_h);
		dev_dbg(&pdev->dev, "ios->clock = %u Hz. host->bus_clock = %u Hz\n",ios->clock,host->bus_clock);
	}
	host->clkreg = clk;
}


static void zmpmci_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	struct zmpmci_host *host = mmc_priv(mmc);
	struct platform_device	*pdev = host->pdev;
 ...
 */
static int zmpmci_get_ro(struct mmc_host *mmc)
{
#ifdef READ_ONLY_SD   
	struct zmpmci_host *host = mmc_priv(mmc);
...
static void zmpmci_enable_sdio_irq(struct mmc_host *mmc, int enable)
{
	__maybe_unused unsigned reg1,reg2;
	unsigned long flags;	
	struct zmpmci_host *host = mmc_priv(mmc);
	struct platform_device	*pdev = host->pdev;
	__maybe_unused static int init_sdio_irq = 0 ;
 ...
 * 
 */
static struct mmc_host_ops zmpmci_ops = {
	.request = zmpmci_request,
	.set_ios = zmpmci_set_ios,
	.get_ro  = zmpmci_get_ro,
	.get_cd  = zmpmci_card_present,
	.enable_sdio_irq = zmpmci_enable_sdio_irq,
};
 ...
 * @return 0:success..
 */
static int zmpmci_init_mmc_host(struct zmpmci_host *host)
{
...
static void zmpmci_pinctrl_idle(struct zmpmci_host *host)
{	
	struct pinctrl_state *state;
	if(host->pdev->dev.pins)
	{
...
static int zmpmci_cpufreq_transition(struct notifier_block *nb,
				     unsigned long val, void *data)
{
	struct zmpmci_host *host;
	struct mmc_host *mmc;
	unsigned long newclk;
	unsigned long flags;
	struct cpufreq_freqs *freqs = (struct cpufreq_freqs *)data;
	host = container_of(nb, struct zmpmci_host, freq_transition);
	struct platform_device	*pdev = host->pdev;
	
	dev_dbg(&pdev->dev, "%s(): in_interrupt()=%ld\n", __func__, in_interrupt());
	dev_dbg(&pdev->dev, "gclk = %ld\n",clk_get_rate(host->clk));
	dev_dbg(&pdev->dev, "freqs->new_cpufreq.gclk = %d\n",freqs->new_cpufreq.gclk);
		  
	mmc = host->mmc;
	newclk = freqs->new_cpufreq.gclk;
		...
		|| (val == CPUFREQ_POSTCHANGE && newclk < host->asic_clock)) 
	{
		...
		if (mmc->ios.power_mode != MMC_POWER_OFF &&
			mmc->ios.clock != 0)
		{		
			dev_dbg(&pdev->dev, "%s(): preempt_count()=%d\n", __func__, preempt_count());
			...
			dev_dbg(&pdev->dev, "MCI_CLK_REG1 = %d\n",readl(host->base + MCI_CLK_REG));	
			aksdio_set_clk(host, &mmc->ios);
			dev_dbg(&pdev->dev, "MCI_CLK_REG2 = %d\n",readl(host->base + MCI_CLK_REG));	
...
static inline int zmpmci_cpufreq_register(struct zmpmci_host *host)
{
	// use for requst and cpufreq
	sema_init(&host->freq_lock, 1);
	
	host->freq_transition.notifier_call = zmpmci_cpufreq_transition;

	return cpufreq_register_notifier(&host->freq_transition,
					 CPUFREQ_TRANSITION_NOTIFIER);
}

static inline void zmpmci_cpufreq_deregister(struct zmpmci_host *host)
{
...
static const struct of_device_id zmpmci_match[] = {
	{ .compatible = "zlgmcu,zmp110x-mmc0" },	
	{ .compatible = "zlgmcu,zmp110x-mmc1" },
	{ .compatible = "zlgmcu,zmp110x-mmc2" },
	{ }
};
MODULE_DEVICE_TABLE(of, zmpmci_match);

static  struct zmpmci_host*
zmpmci_of_init(struct zmpmci_host *host)
{
	struct platform_device *pdev = host->pdev;
	struct device_node *np = pdev->dev.of_node;
	if (!np) {
	...
	if(of_property_read_bool(np, "non-removable"))
		host->detect_mode = ZMPMCI_PLUGIN_ALWAY;
	else if(of_property_read_bool(np, "cd_clk"))
		host->detect_mode = ZMPMCI_DETECT_MODE_CLK;
	else if(of_property_read_bool(np, "cd-gpio") || of_property_read_bool(np, "cd-gpios") || of_property_read_bool(np, "detect-gpio") || of_property_read_bool(np, "detect-gpios"))
		host->detect_mode = ZMPMCI_DETECT_MODE_GPIO;
	else
		host->detect_mode = ZMPMCI_PLUGIN_ALWAY;
	
	host->xfer_mode = ZMPMCI_XFER_L2DMA;
...
#else /* CONFIG_OF */
static inline struct zmpmci_host*
zmpmci_of_init(struct zmpmci_host *host)
{
	return ERR_PTR(-EINVAL);
}
...
static ssize_t mmc_card_pwr_enable_read(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	struct zmpmci_host *host = container_of(pga, struct zmpmci_host, gpio_power);
	unsigned int mmc_en;
...
/*
 * @brief 	ZMPMCI_DETECT_MODE_GPIO mode using timer to detect the change of card status.
 *			This is the handler of timer.
 *
 */
static void zmpmci_detect_change(unsigned long data)
{
	struct zmpmci_host *host = (struct zmpmci_host *)data;
	__maybe_unused unsigned int card_pwr_en;
	int curr_status = 0;
 ...
 *
 */
static ssize_t zmpmci_show_card_status(struct device *dev, struct device_attribute *attr,
				char *buf)
{
	struct zmpmci_host *host;
	struct mmc_host *mmc;
...
static DEVICE_ATTR(card_status, S_IRUGO , zmpmci_show_card_status, NULL);
/*
 * @brief  /sys/devices/platform/soc/20100000.mmc0/bus_clock
 *
 */
static ssize_t zmpmci_show_bus_clock(struct device *dev, struct device_attribute *attr,
				char *buf)
{
	struct zmpmci_host *host;
	struct mmc_host *mmc;
...
static DEVICE_ATTR(bus_clock, S_IRUGO , zmpmci_show_bus_clock, NULL);

/*
 * @brief  /sys/devices/platform/soc/20100000.mmc0/bus_mode
 *
 */
static ssize_t zmpmci_show_bus_mode(struct device *dev, struct device_attribute *attr,
				char *buf)
{
	struct zmpmci_host *host;
	struct mmc_host *mmc;
...
static DEVICE_ATTR(bus_mode, S_IRUGO , zmpmci_show_bus_mode, NULL);

/*
 * @brief  /sys/devices/platform/soc/20100000.mmc0/bus_width
 *
 */
static ssize_t zmpmci_show_bus_width(struct device *dev, struct device_attribute *attr,
				char *buf)
{
	struct zmpmci_host *host;
	struct mmc_host *mmc;
...
static DEVICE_ATTR(bus_width, S_IRUGO , zmpmci_show_bus_width, NULL);

/*
 * @brief  /sys/devices/platform/soc/20100000.mmc0/mci_mode
 *
 */
static ssize_t zmpmci_show_mci_mode(struct device *dev, struct device_attribute *attr,
				char *buf)
{
	struct zmpmci_host *host;
	struct mmc_host *mmc;
...
static DEVICE_ATTR(mci_mode, S_IRUGO , zmpmci_show_mci_mode, NULL);

/*
 * @brief  /sys/devices/platform/soc/20100000.mmc0/f_min
 *
 */
static ssize_t zmpmci_show_f_min(struct device *dev, struct device_attribute *attr,
				char *buf)
{
	struct zmpmci_host *host;
	struct mmc_host *mmc;
...
static DEVICE_ATTR(f_min, S_IRUGO , zmpmci_show_f_min, NULL);

/*
 * @brief  /sys/devices/platform/soc/20100000.mmc0/f_max
 *
 */
static ssize_t zmpmci_show_f_max(struct device *dev, struct device_attribute *attr,
				char *buf)
{
	struct zmpmci_host *host;
	struct mmc_host *mmc;
 ...
 *
 */
static ssize_t zmpmci_store_f_max(struct device *dev, struct device_attribute *attr,const char *buf, size_t count)
{
	struct zmpmci_host *host;
	struct mmc_host *mmc;
	unsigned int f_max;
...
static DEVICE_ATTR(f_max, 0600 , zmpmci_show_f_max, zmpmci_store_f_max);

/*
 * @brief  /sys/devices/platform/soc/20100000.mmc0/detect_mode
 *
 */
static ssize_t zmpmci_show_detect_mode(struct device *dev, struct device_attribute *attr,
				char *buf)
{
	struct zmpmci_host *host;
	struct mmc_host *mmc;
	...
	if(host->detect_mode == ZMPMCI_PLUGIN_ALWAY)
		return sprintf(buf, "%s\n", "non-removable");
	else if(host->detect_mode == ZMPMCI_DETECT_MODE_CLK)
		return sprintf(buf, "%s\n", "clk_detect_mode");
	else if(host->detect_mode == ZMPMCI_DETECT_MODE_GPIO)
		return sprintf(buf, "%s\n", "gpio_detect_mode");
	else
...
static DEVICE_ATTR(detect_mode, 0600 , zmpmci_show_detect_mode, NULL);
				
static struct attribute *zmpmci_attributes[] = {
...
static const struct attribute_group zmpmci_attrs = {
	.attrs = zmpmci_attributes,
};
...
{
	struct zmpmci_host *host =
		container_of(work, struct zmpmci_host, work.work);
	int curr_status = 0;
 ...
 */
static int zmpmci_probe(struct platform_device *pdev)
{
	struct zmpmci_host *host;
	struct mmc_host *mmc;
	struct device_node* np = pdev->dev.of_node;
	...
	mmc = mmc_alloc_host(sizeof(struct zmpmci_host), &pdev->dev);
	if (!mmc) {
		ret = -ENOMEM;
	...
	dev_dbg(&pdev->dev,"zmpmci_probe : %s\n", host->mci_mode?"SDIO":"MCI");

	host->mem = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	host->base = devm_ioremap_resource(&pdev->dev, host->mem);
	...
	if (devm_request_irq(&pdev->dev, host->irq_mci, zmpmci_irq, 0, pdev->name, host)) {
	 ...
	 *		ZMPMCI_PLUGIN_ALWAY
	 *		ZMPMCI_DETECT_MODE_CLK
	 *		ZMPMCI_DETECT_MODE_GPIO
	 */
	if(host->detect_mode == ZMPMCI_DETECT_MODE_CLK)
	{
	...
	ret = zmpmci_cpufreq_register(host);
	if (ret) {
		goto clk_disable;
	}
	...
	sysfs_create_group(&pdev->dev.kobj, &zmpmci_attrs);

	dev_dbg(&pdev->dev, "Mci%d Interface.using %s.%s detect mode:%s.\n",
		 pdev->id,
		 xfer_mode_desc[zmpmci_xfer_mode(host)],
...
static int zmpmci_remove(struct platform_device *pdev)
{
	struct mmc_host *mmc;
	struct zmpmci_host *host;
	...
	sysfs_remove_group(&pdev->dev.kobj, &zmpmci_attrs);
	mmc_remove_host(mmc);
	
	zmpmci_cpufreq_deregister(host);
	...
	if(host->detect_mode == ZMPMCI_DETECT_MODE_GPIO)
...
#ifdef CONFIG_PM
static int zmpmci_suspend(struct device *dev)
{
	struct mmc_host *mmc = platform_get_drvdata(to_platform_device(dev));
	return mmc_suspend_host(mmc);
}

static int zmpmci_resume(struct device *dev)
{
	struct mmc_host *mmc = platform_get_drvdata(to_platform_device(dev));
	return mmc_resume_host(mmc);
}

static struct dev_pm_ops zmpmci_pm = {
	.suspend = zmpmci_suspend,
	.resume = zmpmci_resume
};

#define zmpmci_pm_ops  &zmpmci_pm
#else
#define zmpmci_pm_ops  NULL
#endif

static struct platform_driver zmpmci_driver = {
	.probe = zmpmci_probe,
	.remove = zmpmci_remove,
	.driver 	= {
		.name	= DRIVER_NAME,
		.owner	= THIS_MODULE,
		.pm 	= zmpmci_pm_ops,
		.of_match_table	= of_match_ptr(zmpmci_match),
	},
};
 ...
 */
static int __init zmpmci_init(void)
{
	printk("ZMP MCI Driver  Module_init\n");
	...
	return platform_driver_register(&zmpmci_driver);
}
 ...
 * @return void.
 */
static void __exit zmpmci_exit(void)
{
	platform_driver_unregister(&zmpmci_driver);
	return kobject_put(gpio_en_obj);
}
module_init(zmpmci_init);
module_exit(zmpmci_exit);
