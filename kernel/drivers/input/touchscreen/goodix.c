...
exit:
	dev_dbg(&ts->client->dev, "ts->abs_x_max = %d\n",ts->abs_x_max);
	dev_dbg(&ts->client->dev, "ts->abs_y_max = %d\n",ts->abs_y_max);
	dev_dbg(&ts->client->dev, "ts->int_trigger_type = %u\n",ts->int_trigger_type);
	dev_dbg(&ts->client->dev, "ts->max_touch_num = %u\n",ts->max_touch_num);
...
static int goodix_read_version(struct i2c_client *client, u16 *version, u16 *id)
{
	...
	*version = get_unaligned_le16(&buf[4]);
	...
	if (*id != 911){
		dev_err(&client->dev, "read ID failed: %d\n", *id);
		return -EIO;
	}
			   ...
			   const struct i2c_device_id *id)
{
	...
	dev_dbg(&client->dev,"irq_flags = 0x%lx\n", irq_flags);
