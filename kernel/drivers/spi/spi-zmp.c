...
*  @return      void
*/
static void dbg_dumpregs(struct zmp_spi *hw)
{
	pr_debug("\n");
...
static inline void dbg_dumpdata(struct zmp_spi *hw,
		void *data, int size)
{
	int ii;
	int dsize = (size +3)/4;
...
#define SPI_L2_TXADDR(m)   \
	((m->bus_num == ZMPSPI_BUS_NUM1) ? ADDR_SPI0_TX : ADDR_SPI1_TX)

#define SPI_L2_RXADDR(m)   \
	((m->bus_num == ZMPSPI_BUS_NUM1) ? ADDR_SPI0_RX : ADDR_SPI1_RX)


static inline struct zmp_spi *to_hw(struct spi_device *sdev)
{
...
*  @return      unsigned int
*/
static inline unsigned int hw_txbyte(struct zmp_spi *hw, int len)
{
	u32 val = 0;
	int i = 0;
...
*  @return      unsigned int
*/
static unsigned int hw_txdword(struct zmp_spi *hw)
{
	u32 val = 0;
	int l = 0;
...
*  @return      unsigned int
*/
static inline void hw_rxbyte(struct zmp_spi *hw, unsigned int val, int len)
{
	int i = 0;
...
*  @return      unsigned int
*/
static void hw_rxdword(struct zmp_spi *hw, unsigned int val)
{
	int l = 0;
...
static inline u32 hw_remain_datalen(struct zmp_spi *hw)
{
	return (hw->len - hw->count);
}

static inline bool zmp_spi_use_dma(struct zmp_spi *hw)
{
	return (hw->xfer_mode == ZMPSPI_XFER_MODE_DMA);
}

static inline int wait_for_spi_cnt_to_zero(struct zmp_spi *hw, u32 timeout)
{
	do {			 
		if (readl(hw->regs + ZMP_SPICNT) == 0)
			break;
		udelay(1);
	}while(timeout--);
...
static void zmp_spi_set_cs(struct zmp_spi *hw, int cs, int pol)
{
	BUG_ON(cs >= 1);
}

/**
*  @brief       zmp_spi_chipsel
*  @author      zhou wenyong
*  @date        2011-08-19
*  @param[out]  *spi
*  @param[in]   value
*  @return      void
*/
static void zmp_spi_chipsel(struct spi_device *spi, int value)
{
	struct zmp_spi_devstate *cs = spi->controller_state;
	struct zmp_spi *hw = to_hw(spi);
	unsigned int cspol = (spi->mode & SPI_CS_HIGH) ? 1 : 0;
	
	/* change the chipselect state and the state of the spi engine clock */	
	switch (value) {
		case CS_INACTIVE:
			if(spi->chip_select == ZMPSPI_ONCHIP_CS) {
				cs->spcon = ioread32(hw->regs + ZMP_SPICON);
				cs->spcon &= ~FORCE_CS;
				iowrite32(cs->spcon, hw->regs + ZMP_SPICON);
			}
			else
				zmp_spi_set_cs(hw, spi->chip_select, cspol^1);
		...
		case CS_ACTIVE:
			...
			if(spi->chip_select == ZMPSPI_ONCHIP_CS) {
				...
				zmp_spi_set_cs(hw, spi->chip_select, cspol);
			break;
		default:
			break;
	}
...
*  @return      int
*/
static int zmp_spi_update_state(struct spi_device *spi,
				    struct spi_transfer *t)
{
	...
	if (spi->mode != cs->mode) {
		cs->spcon &= ~(ZMP_SPICON_CPHA|ZMP_SPICON_CPOL);
		
		if ((spi->mode & SPI_CPHA) == SPI_CPHA)
			cs->spcon |= ZMP_SPICON_CPHA;

		if ((spi->mode & SPI_CPOL) == SPI_CPOL)
			cs->spcon |= ZMP_SPICON_CPOL;
...
*  @return      int
*/
static int zmp_spi_setupxfer(struct spi_device *spi,
				 struct spi_transfer *t)
{
	struct zmp_spi_devstate *cs = spi->controller_state;
	struct zmp_spi *hw = to_hw(spi);
	...
	cs->spcon = readl(hw->regs + ZMP_SPICON);

	ret = zmp_spi_update_state(spi, t);
	if (!ret)
	{
	    /*Add code to fix the issue that the SPICON register 
	          was deranged. Shaohua @2012-3-23	           
	     */
		writel(cs->spcon, hw->regs + ZMP_SPICON);
	    //printk("zmp_spi_setupxfer,con:%08x.\n", cs->spcon);
		cs->initialed = 1;
		hw->curdev = spi;
	}
...
*/
static int zmp_spi_setup(struct spi_device *spi)
{
	struct zmp_spi_devstate *cs = spi->controller_state;
	struct zmp_spi *hw = to_hw(spi);
	int ret;
	...
	if (!cs) {
		cs = kzalloc(sizeof(struct zmp_spi_devstate), GFP_KERNEL);
		if (!cs) {
	...
	ret = zmp_spi_update_state(spi, NULL);
...
static void zmp_spi_cleanup(struct spi_device *spi)
{
	kfree(spi->controller_state);
}

static inline u32 enable_imask(struct zmp_spi *hw, u32 imask)
{
	u32 newmask;

	newmask = readl(hw->regs + ZMP_SPIINT);
	newmask |= imask;

	writel(newmask, hw->regs + ZMP_SPIINT);

	return newmask;
}

static inline u32 disable_imask(struct zmp_spi *hw, u32 imask)
{
	u32 newmask;
...
*  @author      lixinhai
*  @date        2013-03-15
*  @param[in]  *hw :zmpspi master dev.
*  @param[in]   t: a read/write buffer pair.
*  @return      void
*/
static void zmp_spi_start_txrx(struct zmp_spi *hw, struct spi_transfer *t)
{
    u32 reg_value;
    ...
    reg_value = readl(hw->regs + ZMP_SPICON);
    ...
    writel(reg_value, hw->regs + ZMP_SPICON);
}
...
*  @return      void
*/
static void zmp_spi_stop_txrx(struct zmp_spi *hw, struct spi_transfer *t)
{
	u32 reg_value;
...
*/
static int spi_dma_write(struct zmp_spi *hw, unsigned char *buf, int count)
{
	int ret = 0;
	bool flags = false;
	u32 val;
	...
	enable_imask(hw, ZMP_SPIINT_TRANSF);
	
	val = ZMP_SPIEXTX_BUFEN|ZMP_SPIEXTX_DMAEN;
	iowrite32(val, hw->regs + ZMP_SPIEXTX);
	iowrite32(count, hw->regs + ZMP_SPICNT);
	...
	if((count < 256) || ((unsigned long)buf & 0x3) || (count & (64 - 1))) {
		...
		l2_combuf_dma(hw->txdma_buffer, hw->l2buf_tid, count, MEM2BUF, ZMP_FALSE);
		flags = true;
	}
	...
	if (flags && (l2_combuf_wait_dma_finish(hw->l2buf_tid) == ZMP_FALSE))	{
...
xfer_fail:
	...
	//disable l2 dma
	iowrite32(0, hw->regs + ZMP_SPIEXTX);
...
*  data from l2 buffer to DMA buffer.
*/
static int spi_dma_read(struct zmp_spi *hw, unsigned char *buf, int count)
{
	int ret = 0;
	bool flags = false;
	u32 val;
	...
	val = ZMP_SPIEXTX_BUFEN|ZMP_SPIEXTX_DMAEN;
	iowrite32(val, hw->regs + ZMP_SPIEXRX);
	iowrite32(count, hw->regs + ZMP_SPICNT);	
	...
	if(count < 256 || ((unsigned long)buf & 0x3) || (count & (64 - 1))) {
		...
		/* data from l2 buffer to dma buffer*/
		l2_combuf_dma(hw->rxdma_buffer, hw->l2buf_rid, count, BUF2MEM, ZMP_FALSE);
		flags = true;
	}
	...
	if (flags && l2_combuf_wait_dma_finish(hw->l2buf_rid) ==  ZMP_FALSE)	{
		pr_err("%s: l2_combuf_wait_dma_finish failed!\n", __func__);
		ret = -EINVAL;
		goto xfer_fail;
	}
...
xfer_fail:
	//disable l2 dma
	iowrite32(0, hw->regs + ZMP_SPIEXRX);
	l2_clr_status(hw->l2buf_rid);
	return ret;
}

/* spi read/write date function by l2dma mode*/
static int spi_dma_duplex(struct zmp_spi *hw, 
			unsigned char *tx, unsigned char *rx, int count)
{
	...
	val = ZMP_SPIEXRX_BUFEN|ZMP_SPIEXRX_DMAEN;
	iowrite32(val, hw->regs + ZMP_SPIEXRX);
	...
	if (flags && ((ZMP_FALSE == l2_combuf_wait_dma_finish(hw->l2buf_tid)) || 
		(ZMP_FALSE == l2_combuf_wait_dma_finish(hw->l2buf_rid))))
	{
		pr_err("%s: l2_combuf_wait_dma_finish failed!\n", __func__);
		ret = -EINVAL;
		goto xfer_fail;
...
*/
static int zmp_spi_dma_txrx(struct zmp_spi *hw, struct spi_transfer *t)
{
	int ret = 0;
	u32 retlen;
	u32 count = t->len;
...
*  @return      transfer success result 0, otherwise result a negative value
*/
static int zmp_cpu_duplex(struct spi_device *spi, struct spi_transfer *t)
{
	struct zmp_spi *hw = to_hw(spi);
	u32 tran_4_nbr;
	u32 frac_nbr;
	...
	writel(hw->len, hw->regs + ZMP_SPICNT);
	
	while(1) {
		/*write 4 bytes first, and then read 4 bytes*/
		if (i<tran_4_nbr) {
			while(1) {
				status = ioread32(hw->regs + ZMP_SPISTA);
			...
			iowrite32(*(volatile u32 *)(buff_tx + off_set_write), hw->regs + ZMP_SPIOUT);
			off_set_write += 4;
			i++;
		}
		//write not finished
			...
			if (frac_nbr != 0) {
				...
				iowrite32(val, hw->regs + ZMP_SPIOUT); 
		...
		if ((status & ZMP_SPISTA_TRANSF) == ZMP_SPISTA_TRANSF) {
			if (status & ZMP_SPISTA_RXFULL) {
				val = ioread32(hw->regs + ZMP_SPIIN);
...
static int pio_imasks[SPI_DIR_XFER_NUM] = {
	ZMP_SPIINT_TRANSF | ZMP_SPIINT_TXHEMP,
	ZMP_SPIINT_TRANSF | ZMP_SPIINT_RXHFULL,
	ZMP_SPIINT_TXHEMP | ZMP_SPIINT_RXHFULL,
};
...
*  @return      transfer success result 0, otherwise result a negative value
*/
static int zmp_spi_pio_txrx(struct zmp_spi *hw, struct spi_transfer *t)
{
	int ret;
	printk("spi txrx\n");
//	reinit_completion(&hw->done);
...
static int spi_pio_write(struct zmp_spi *hw, unsigned char *buf, int count)
{
	u32 status;
	u32 to = 0;

	pr_debug("zmp spi write by cpu mode\n");
	if (count > 64*1024)
	{
	...
	//set data count, and the the master will rise clk
	writel(count, hw->regs + ZMP_SPICNT);
	
	while(hw_remain_datalen(hw) > 0) {
		status = readl(hw->regs + ZMP_SPISTA);
		if ((status & ZMP_SPISTA_TXHEMP) == ZMP_SPISTA_TXHEMP) {
			writel(hw_txdword(hw), hw->regs + ZMP_SPIOUT);
		}else {
			if (to++ > 10 * 1000000) {
...
*  @return      int
*/
static int spi_pio_read(struct zmp_spi *hw, unsigned char *buf, int count)
{
	u32 status;
	u32 to=0;
	
	pr_debug("zmp spi read by cpu mode\n");
	if (count >= 64*1024)
	{
	...
	while(1) {
		...
		{
			if(status & ZMP_SPISTA_RXFULL) {
				hw_rxdword(hw, readl(hw->regs + ZMP_SPIIN));
				hw_rxdword(hw, readl(hw->regs + ZMP_SPIIN));
			}else if (status & ZMP_SPISTA_RXHFULL) {
				hw_rxdword(hw, readl(hw->regs + ZMP_SPIIN));
			} 
...
*  @return      int
*/
static int zmp_spi_txrx(struct spi_device *spi, struct spi_transfer *t)
{
    int ret;
	struct zmp_spi *hw = to_hw(spi);
	...
	hw->xfer_mode = t->len < 256 ? ZMPSPI_XFER_MODE_CPU : ZMPSPI_XFER_MODE_DMA;

	hw->xfer_dir = (t->tx_buf && t->rx_buf) ? SPI_DIR_TXRX : 
				t->tx_buf ? SPI_DIR_TX : SPI_DIR_RX;

	zmp_spi_start_txrx(hw, t);
	
	if(hw->xfer_dir == SPI_DIR_TXRX) {		
		ret = zmp_cpu_duplex(spi, t);
		return ret;
	}	
	if(0){
//	if(zmp_spi_use_dma(hw)) {
		ret = zmp_spi_dma_txrx(hw, t);
	} else {
		ret = zmp_spi_pio_txrx(hw, t);
	}

//	zmp_spi_stop_txrx(hw, t);
...
*  @return      irqreturn_t
*/
static irqreturn_t zmp_spi_irq(int irq, void *dev)
{
	struct zmp_spi *hw = dev;
	unsigned int status;
...
//	if(zmp_spi_use_dma(hw)) {
		if((status & ZMP_SPISTA_TRANSF) == ZMP_SPISTA_TRANSF ) {
			printk("spi transfer data have been finish.\n"); 
			disable_imask(hw, ZMP_SPIINT_TRANSF);
			complete(&hw->done);
		}
	} else {
		switch(hw->xfer_dir) {	
			case SPI_DIR_RX:
				if(status & (ZMP_SPISTA_RXHFULL | ZMP_SPISTA_TRANSF)) {
					pr_debug("spi recv buffer half full or data transfer finish.\n");

					if(status & (ZMP_SPISTA_RXFULL|ZMP_SPISTA_TRANSF))
						hw_rxdword(hw, readl(hw->regs + ZMP_SPIIN));						
					hw_rxdword(hw, readl(hw->regs + ZMP_SPIIN));

					if (hw->count >= hw->len) {
						disable_imask(hw, ZMP_SPIINT_RXHFULL|ZMP_SPIINT_TRANSF);
//						complete(&hw->done);
					}				
				}
				break;
			case SPI_DIR_TX:
				if(status & (ZMP_SPISTA_TXHEMP | ZMP_SPISTA_TRANSF)) {
					pr_debug("spi send buffer half empty or data transfer finish.\n");
					
					if (hw->count < hw->len) {
						if((status & ZMP_SPISTA_TXEMP) && ((hw->count - hw->len)>4))
							writel(hw_txdword(hw), hw->regs + ZMP_SPIOUT);
						
						writel(hw_txdword(hw), hw->regs + ZMP_SPIOUT);
					} else {
						disable_imask(hw, ZMP_SPIINT_TXHEMP|ZMP_SPIINT_TRANSF);
//						complete(&hw->done);
					}
				}
				break;
...
*  @return      void
*/
static void zmp_spi_initialsetup(struct zmp_spi *hw)
{
	BUG_ON(hw->master && hw->master->bus_num >= ZMPSPI_MAX_BUS_NUM);

	/* program defaults into the registers */
	writel(DFT_DIV<<8 | DFT_CON | (1<<1),hw->regs + ZMP_SPICON);

	writel(0, hw->regs + ZMP_SPIINT);
}
...
* date:   2018-12-10
*/
static int zmp_spi_transfer_one(struct spi_master *master,
				    struct spi_device *spi,
				    struct spi_transfer *transfer)
{	
    int status = 0;

    status = zmp_spi_setupxfer(spi, transfer);
	if (status < 0)
	{
    ...
    if (transfer->len)
		status = zmp_spi_txrx(spi, transfer);
...
static void zmp_spi_set_chipsel(struct spi_device *spi, bool enable)
{
	enable = (!!(spi->mode & SPI_CS_HIGH) == enable);

	ndelay(100);
	zmp_spi_chipsel(spi, enable ? CS_ACTIVE :
			    CS_INACTIVE);
	ndelay(100);
}
...
*  @return      int __init
*/
static int  zmp_spi_probe(struct platform_device *pdev)
{
	struct zmp_spi *hw;
	struct spi_master *master;
	struct resource *res;
	...
	master = spi_alloc_master(&pdev->dev, sizeof(struct zmp_spi));
	if (master == NULL) 
	{
	...
	memset(hw, 0, sizeof(struct zmp_spi));

	hw->master = spi_master_get(master);
	hw->dev = &pdev->dev;
	hw->xfer_mode = ZMPSPI_XFER_MODE_CPU;
    ...
    master->setup  = zmp_spi_setup;
    master->cleanup = zmp_spi_cleanup;
    master->set_cs = zmp_spi_set_chipsel;
    master->transfer_one = zmp_spi_transfer_one;
	...
	err = request_irq(hw->irq, zmp_spi_irq, 0, pdev->name, hw);
	if (err) {
	...
	pr_debug("zmpspi master SPI%d initialize success, use for %s mode.\n", 
		master->bus_num, zmp_spi_use_dma(hw)?"DMA":"PIO");
...
static int __exit zmp_spi_remove(struct platform_device *dev)
{
	struct zmp_spi *hw = platform_get_drvdata(dev);
...
#ifdef CONFIG_PM
static int zmp_spi_suspend(struct device *dev)
{
	struct zmp_spi *hw = platform_get_drvdata(to_platform_device(dev));

	zmp_spi_gpio_setup(hw, 0);

	clk_disable(hw->clk);
	return 0;
}

static int zmp_spi_resume(struct device *dev)
{
	struct zmp_spi *hw = platform_get_drvdata(to_platform_device(dev));
...
static struct dev_pm_ops zmp_spi_pmops = {
	.suspend	= zmp_spi_suspend,
	.resume		= zmp_spi_resume,
};

#define ZMP_SPI_PMOPS &zmp_spi_pmops
#else
#define ZMP_SPI_PMOPS NULL
#endif /* CONFIG_PM */

static const struct of_device_id zmp_spi_match[] = {
	{ .compatible = "zlgmcu,zmp110x-spi0", },
	{ .compatible = "zlgmcu,zmp110x-spi1", },
	{}
};
MODULE_DEVICE_TABLE(of, zmp_spi_match);

MODULE_ALIAS("platform:zmp-spi");
static struct platform_driver zmp_spi_driver = {
    .probe          = zmp_spi_probe,
	.remove		= __exit_p(zmp_spi_remove),
	.driver		= {
		.name	= "zmp-spi",
        .of_match_table	= of_match_ptr(zmp_spi_match),
		.owner	= THIS_MODULE,
		.pm	= ZMP_SPI_PMOPS,
	},
};

static int __init zmp_spi_init(void)
{
        return platform_driver_register(&zmp_spi_driver);
}

static void __exit zmp_spi_exit(void)
{
        platform_driver_unregister(&zmp_spi_driver);
}

module_init(zmp_spi_init);
module_exit(zmp_spi_exit);
MODULE_DESCRIPTION("ZMP On-Chip SPI controller driver");
