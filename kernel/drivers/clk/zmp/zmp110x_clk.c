...
/**
 * DOC: ZMP110x (clock resource manager)
 *
 * The clock tree on the ZMP110x has several levels.  There's a root
 * oscillator running at 24Mhz. After the oscillator there are 3
...
#define ZMP_CLKCON_CLK_DAC			(1 << 28)
#define ZMP_CLKCON_CLK_ADC1			(1 << 3)

/* ADC2/DAC high speed clock control register */
#define ZMP_CLKCON_HCLK_ADC2			(1 << 28)
#define ZMP_CLKCON_HCLK_DAC			(1 << 18)
#define ZMP_CLKCON_CLK_ADC2			(1 << 8)


/* PERI PLL channel clock control register0 */
//1: peri pll 	0: external 12MHz
#define ZMP_CLKCON_CLK_PHY_SEL		(1 << 19)
//1: peri pll	0: external 25MHz
#define ZMP_CLKCON_CLK_MAC_SEL		(1 << 18)
#define ZMP_CLKCON_CLK_12M			(1 << 17)
#define ZMP_CLKCON_CLK_25M			(1 << 16)
#define ZMP_CLKCON_CLK_25M_IN		(1 << 14)

/* PERI PLL channel clock control register1 */
/* 1: positive clk	0: negative clk */
#define ZMP_CLKCON_PCLK_CIS			(1 << 28)		//camera
#define ZMP_CLKCON_SCLK_CIS			(1 << 18)		//sensor
#define ZMP_CLKCON_CLK_OPCLK			(1 << 8)		//MAC


#define ZMP110X_CLK_CPU2X4X_MODE		(1 << 24)
#define ZMP110X_CLK_CPU_DPHY_OPP_EN	(1 << 26)
#define ZMP110X_CLK_DPHY1X2X_MODE		(1 << 30)

#define MHz	1000000UL

struct zmp_fixed_clk {
	int id;
	struct clk_hw hw;
	void __iomem *reg;
...
#define to_clk_zmp_fixed(_hw) container_of(_hw, struct zmp_fixed_clk, hw)
#define to_clk_zmp_factor(_hw) container_of(_hw, struct zmp_factor_clk, hw)
#define to_clk_zmp_mux(_hw) container_of(_hw, struct zmp_mux_clk, hw)
#define to_clk_zmp_gate(_hw) container_of(_hw, struct zmp_gate_clk, hw)
...
static unsigned long zmp_get_cpu_pll_clk(void __iomem *reg)
{
...
static unsigned long zmp_get_core_pll_clk(void __iomem *reg)
{
    u32 pll_m, pll_n, pll_od;
    u32 core_pll_clk;
    u32 regval;
...
static unsigned long zmp_get_vclk(void __iomem *reg)
{
   u32 regval;
   u32 vclk;
   regval = __raw_readl(reg + CLOCK_CORE_PLL_CTRL);
   //修复vclk驱动代码错误：逻辑与&&错误写作位与&，hash：c13839a3。
   regval = ((regval >> 17) & 0x7);
   if (regval == 0)
	   regval = 1;
   vclk = (zmp_get_core_pll_clk(reg)/MHz)>>(regval);

   return vclk * MHz;
}


static long zmp_fixed_clk_round_rate(struct clk_hw *hw, unsigned long rate,
					unsigned long *parent_rate)
{
	struct zmp_fixed_clk *fixed_clk = to_clk_zmp_fixed(hw);
...
static int zmp_fixed_clk_set_rate(struct clk_hw *hw,
...
static unsigned long zmp_fixed_clk_recalc_rate(struct clk_hw *hw, unsigned long parent_rate)
{
	u32 regval;
	...
	if (regval & ZMP110X_CLK_CPU_DPHY_OPP_EN)
	{
		div_j = 0;
		div_d = 1;
		div_phy = 0;
	} else {
		if (regval & ZMP110X_CLK_CPU2X4X_MODE) {
			div_j = 0;
			div_d = 2;
		} else {
			div_j = 1;
		...
		if (regval & ZMP110X_CLK_DPHY1X2X_MODE)
			div_phy = 1;
		else
			div_phy = 2;
	}
	...
	switch(fixed_clk->id) {
		case 0:	//JCLK
			fixed_clk->fixed_rate = zmp_get_cpu_pll_clk(fixed_clk->reg) >> div_j;
			pr_err("CPU(JCLK): %lu(Mhz) \n", fixed_clk->fixed_rate/MHz);
			break;
		case 1: //HCLK/DCLK
			fixed_clk->fixed_rate = zmp_get_cpu_pll_clk(fixed_clk->reg) >> div_d;
			//pr_err("MEMCTRL(HCLK/DCLK): %lu(Mhz) \n", fixed_clk->fixed_rate/MHz);
			break;
		case 2: //DPHY
			fixed_clk->fixed_rate = zmp_get_cpu_pll_clk(fixed_clk->reg) >> div_phy;
			pr_err("MEMDDR2(DPHY): %lu(Mhz) \n", fixed_clk->fixed_rate/MHz);
			break;
		case 3: //CORE_PLL set in init func, just return
			fixed_clk->fixed_rate = zmp_get_core_pll_clk(fixed_clk->reg);
...
const struct clk_ops zmp_fixed_clk_ops = {
	.round_rate = zmp_fixed_clk_round_rate,
	.set_rate = zmp_fixed_clk_set_rate,
	.recalc_rate = zmp_fixed_clk_recalc_rate,
};
...
static int sdadc_set_div_osr(struct zmp_factor_clk *factor_clk,
		unsigned long rate, unsigned long *parent_rate)
{
	int div, osr, map_osr;
	int timeout;
...
static int sddac_set_div_osr(struct zmp_factor_clk *factor_clk, unsigned long rate, unsigned long *parent_rate)
{
	int div, osr, map_osr;
	int timeout;
	u32 regval;
...
static int adchs_set_div(struct zmp_factor_clk *factor_clk, int factor)
{
	int timeout;
	u32 regval;
...
static int dachs_set_div(struct zmp_factor_clk *factor_clk, int factor)
{
	int timeout;
	u32 regval;
...
static long zmp_factor_clk_round_rate(struct clk_hw *hw, unsigned long rate,
					unsigned long *parent_rate)
{
	struct zmp_factor_clk *factor_clk = to_clk_zmp_factor(hw);
...
static int zmp_factor_clk_set_rate(struct clk_hw *hw,
	unsigned long rate, unsigned long parent_rate)
{
	struct zmp_factor_clk *factor_clk = to_clk_zmp_factor(hw);
	u32 regval;
...
static unsigned long sdadc_get_clk(struct zmp_factor_clk *factor_clk,
		unsigned long parent_rate)
{
	int div, osr;
	unsigned int regval_div, regval_osr;
	unsigned long sdadc_clk;
	void __iomem *reg = factor_clk ? (factor_clk->reg): ZMP_VA_SYSCTRL;
...
static unsigned long sddac_get_clk(struct zmp_factor_clk *factor_clk, unsigned long parent_rate)
{
	int div, osr;
	unsigned int regval_div, regval_osr;
...
static unsigned long zmp_factor_clk_recalc_rate(struct clk_hw *hw,
					unsigned long parent_rate)
{
	struct zmp_factor_clk *factor_clk = to_clk_zmp_factor(hw);
	u32 regval;
...
const struct clk_ops zmp_factor_clk_ops = {
	.round_rate = zmp_factor_clk_round_rate,
	.set_rate = zmp_factor_clk_set_rate,
	.recalc_rate = zmp_factor_clk_recalc_rate,
};

static int zmp_mux_clk_set_parent(struct clk_hw *hw, u8 index)
{
	//struct zmp_client_clk *client_clk = to_clk_zmp_mux(hw);
	/* to do */
	return 0;
}

static u8 zmp_mux_clk_get_parent(struct clk_hw *hw)
{
	//struct zmp_client_clk *client_clk = to_clk_zmp_mux(hw);
	/* to do */
	return 0;
...
const struct clk_ops zmp_mux_clk_ops = {
    .set_parent = zmp_mux_clk_set_parent,
    .get_parent = zmp_mux_clk_get_parent,
    //.determine_rate = __clk_mux_determine_rate,
};

static int zmp_gc_enable(struct clk_hw *hw)
{
	struct zmp_gate_clk *gate_clk = to_clk_zmp_gate(hw);
	u32 con;
	unsigned long flags;

	local_irq_save(flags);
	if (gate_clk->reg == ZMP_VA_SYSCTRL) {
		...
		con &= ~(0x1 << gate_clk->ctrlbit);
		__raw_writel(con, gate_clk->reg + CLOCK_SOFT_RESET);
	} else if (gate_clk->reg == ZMP_VA_SYSCTRL + CLOCK_GATE_RESET_CTRL2) {
...
static void zmp_gc_disable(struct clk_hw *hw)
{
	struct zmp_gate_clk *gate_clk = to_clk_zmp_gate(hw);
	u32 con;
	unsigned long flags;
...
static unsigned long zmp_gc_recalc_rate(struct clk_hw *hw,
					unsigned long parent_rate)
{
	struct zmp_gate_clk *gate_clk = to_clk_zmp_gate(hw);
	unsigned long rate;
	u32 regval = __raw_readl(ZMP_VA_SYSCTRL + CLOCK_CORE_PLL_CTRL);
	...
	if(gate_clk->id >= 0 || gate_clk->id <= 28)
	{
		...
		default:
			rate = parent_rate>>(regval);
			regval = __raw_readl(ZMP_VA_SYSCTRL + CLOCK_CORE_PLL_CTRL);
...
const struct clk_ops zmp_gate_clk_ops = {
	.enable		= zmp_gc_enable,
	.disable	= zmp_gc_disable,
	.recalc_rate = zmp_gc_recalc_rate,
};

struct clk * __init zmp_register_fixed_clk(
		const char *name,
...
{
	struct zmp_fixed_clk *fixed_clk;
	struct clk *clk;
...
struct clk * __init zmp_register_factor_clk(
		const char *name,
		const char *parent_name,
		void __iomem *res_reg,
		int id,
		const struct clk_ops *ops)
{
	struct zmp_factor_clk *factor_clk;
	struct clk *clk;
	struct clk_init_data init = {};
...
struct clk * __init zmp_register_mux_clk(
		const char *name,
...
struct clk * __init zmp_clk_register_gate(
		const char *name,
		const char *parent_name,
		void __iomem *res_reg,
		u8 ctrlbit,
...
static void __init of_zmp_fixed_clk_init(struct device_node *np)
{
	struct clk_onecell_data *clk_data;
	const char *clk_name = np->name;
	const char *parent_name = of_clk_get_parent_name(np, 0);
	void __iomem *res_reg = ZMP_VA_SYSCTRL;
	...
	for (index = 0; index < number; index++) {
		...
		pr_debug("of_zmp_fixed_clk_init %s %s %d %d\n", clk_name, parent_name, rate, id);
		clk_data->clks[index] = zmp_register_fixed_clk(clk_name, parent_name,
						res_reg, rate, id, &zmp_fixed_clk_ops);
		if (IS_ERR(clk_data->clks[index])) {
			pr_err("of_zmp_fixed_clk_init register fixed clk failed: clk_name:%s, index = %d\n",
					clk_name, index);
			WARN_ON(true);
			continue;
		}
...
static void __init of_zmp_factor_clk_init(struct device_node *np)
{
	struct clk_onecell_data *clk_data;
	const char *clk_name = np->name;
	const char *parent_name = of_clk_get_parent_name(np, 0);
	...
	for (index = 0; index < number; index++) {
		of_property_read_string_index(np, "clock-output-names",
					      index, &clk_name);
		of_property_read_u32_index(np, "clock-id", index, &id);
		pr_debug("of_zmp_factor_clk_init %s %s %d %d\n", clk_name, parent_name, id, index);
		clk_data->clks[index] = zmp_register_factor_clk(clk_name, parent_name,
						res_reg, id, &zmp_factor_clk_ops);
		if (IS_ERR(clk_data->clks[index])) {
			pr_err("of_zmp_factor_clk_init register gate clk failed: clk_name:%s, err = %s\n",
...
static void __init of_zmp_mux_clk_init(struct device_node *np)
{
	struct clk *clk = NULL;
	const char *clk_name = np->name;
	...
	//pr_err("of_zmp_mux_clk_init %s %s %s %d\n", clk_name, parent_name[0], parent_name[1], id);
	clk = zmp_register_mux_clk(clk_name, parent_name, number,
						res_reg, id, &zmp_mux_clk_ops);
	if (!IS_ERR(clk)) {
		of_clk_add_provider(np, of_clk_src_simple_get, clk);
	} else
		pr_err("of_zmp_mux_clk_init register mux clk failed: clk_name:%s %s \n",
				clk_name, (char *)PTR_ERR(clk));
}

static void __init of_zmp_gate_clk_init(struct device_node *np)
{
	struct clk_onecell_data *clk_data;
	const char *clk_name = np->name;
	const char *parent_name = of_clk_get_parent_name(np, 0);
	...
	for (index = 0; index < number; index++) {
		...
		if (ctrlbit >= 32) {
			ctrlbit %= 32;
			reg = ZMP_VA_SYSCTRL + CLOCK_GATE_RESET_CTRL2;
		}

		pr_debug("of_zmp_gate_clk_init %s %s %d\n", clk_name, parent_name, ctrlbit);
		clk_data->clks[index] = zmp_clk_register_gate(clk_name, parent_name, 
						  reg, ctrlbit, &zmp_gate_clk_ops);
		if (IS_ERR(clk_data->clks[index])) {
			pr_err("of_zmp_gate_clk_init register gate clk failed: clk_name:%s, index = %d\n",
					clk_name, index);
			WARN_ON(true);
			continue;
		}
...
static void __init of_zmp_uv_det_pd(struct device_node *np)
{
	int ret;
	u32 regval;
...
CLK_OF_DECLARE(zmp110x_core_fixed_clk, "zlgmcu,zmp110x-fixed-clk", of_zmp_fixed_clk_init);
CLK_OF_DECLARE(zmp110x_factor_clk, "zlgmcu,zmp110x-factor-clk", of_zmp_factor_clk_init);
CLK_OF_DECLARE(zmp110x_core_mux_clk, "zlgmcu,zmp110x-mux-clk", of_zmp_mux_clk_init);
CLK_OF_DECLARE(zmp110x_core_gate_clk, "zlgmcu,zmp110x-gate-clk", of_zmp_gate_clk_init);
CLK_OF_DECLARE(zmp110x_uv_det_pd, "zlgmcu,zmp110x-uv_det_pd", of_zmp_uv_det_pd);
...
static int proc_clk_show(struct seq_file *m, void *v)
{
	seq_printf(m,
			   ...
			   "dac_rate: 	%lu	Hz\n"	
				,
			   	zmp_get_core_pll_clk(ZMP_VA_SYSCTRL)/MHz,
			   	zmp_get_vclk(ZMP_VA_SYSCTRL)/MHz,
				sdadc_get_clk(NULL, zmp_get_core_pll_clk(ZMP_VA_SYSCTRL)),
				sddac_get_clk(NULL, zmp_get_core_pll_clk(ZMP_VA_SYSCTRL))
				);
