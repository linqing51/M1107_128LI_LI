...
#if defined(CONFIG_GPIO_WATCHDOG)

#define GPIO_DIR_REG1 0x20170000
#define GPIO_OUT_REG1 0x20170014

STATIC void watchdog_reset(int reset)
{
       unsigned int value = 0;
       value = *(volatile unsigned int *)GPIO_OUT_REG1;
       if(reset)
               value |= 1 << 3;
       else
               value &= ~(1 << 3);

       *(volatile unsigned int *)GPIO_OUT_REG1 = value;
}

STATIC void INIT watchdog_pin_init(void)
{
       unsigned int value = 0;
       value = *(volatile unsigned int *)GPIO_DIR_REG1;
       value |= 1 << 3;
       *(volatile unsigned int *)GPIO_DIR_REG1 = value;
