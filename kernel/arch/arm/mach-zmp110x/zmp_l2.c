/*
 * /linux/arch/arm/mach-zmp110x/zmp_l2.c
 *
...
#define vL2DMA_ADDRBUF0         (ZMP_VA_L2CTRL + 0x00)
#define vL2DMA_CONBUF0          (ZMP_VA_L2CTRL + 0x40)

#define L2_DMA_ADDR				(ZMP_VA_L2CTRL + 0x00)
#define L2_DMA_CON				(ZMP_VA_L2CTRL + 0x40)
#define L2_DMAREQ				(ZMP_VA_L2CTRL + 0x80)
#define L2_FRACDMAADDR			(ZMP_VA_L2CTRL + 0x84)
#define L2_CONBUF0_7			(ZMP_VA_L2CTRL + 0x88)  
#define L2_CONBUF8_15			(ZMP_VA_L2CTRL + 0x8C) 
#define L2_BUFASSIGN1			(ZMP_VA_L2CTRL + 0x90) 
#define L2_BUFASSIGN2			(ZMP_VA_L2CTRL + 0x94) 
#define L2_LDMACON				(ZMP_VA_L2CTRL + 0x98) 
#define L2_BUFINTEN				(ZMP_VA_L2CTRL + 0x9C) 
#define L2_BUFSTAT1				(ZMP_VA_L2CTRL + 0xA0) 
#define L2_BUFSTAT2				(ZMP_VA_L2CTRL + 0xA8) 

/*************************** L2 MEMORY CONTROL *********************/
#define rL2_DMAREQ              (ZMP_VA_L2CTRL + 0x80)
#define rL2_FRACDMAADDR         (ZMP_VA_L2CTRL + 0x84)
#define rL2_CONBUF0_7           (ZMP_VA_L2CTRL + 0x88)  
#define rL2_CONBUF8_15          (ZMP_VA_L2CTRL + 0x8C) 
#define rL2_BUFASSIGN1          (ZMP_VA_L2CTRL + 0x90) 
#define rL2_BUFASSIGN2          (ZMP_VA_L2CTRL + 0x94) 
#define rL2_LDMACON             (ZMP_VA_L2CTRL + 0x98) 
#define rL2_BUFINTEN            (ZMP_VA_L2CTRL + 0x9C) 
#define rL2_BUFSTAT1            (ZMP_VA_L2CTRL + 0xA0) 
#define rL2_BUFSTAT2			(ZMP_VA_L2CTRL + 0xA8) 

/*************************** L2 MEMORY BUFFER **********************/
#define rL2_ADDRBUF0            (ZMP_VA_L2MEM + 0x0000)
#define rL2_ADDRBUF1            (ZMP_VA_L2MEM + 0x0200)
#define rL2_ADDRBUF2            (ZMP_VA_L2MEM + 0x0400)
#define rL2_ADDRBUF3            (ZMP_VA_L2MEM + 0x0600)
#define rL2_ADDRBUF4            (ZMP_VA_L2MEM + 0x0800)
#define rL2_ADDRBUF5            (ZMP_VA_L2MEM + 0x0A00)
#define rL2_ADDRBUF6            (ZMP_VA_L2MEM + 0x0C00)
#define rL2_ADDRBUF7            (ZMP_VA_L2MEM + 0x0E00)
...
{
	pr_debug("L2 Register Dumping Begin:\n");
	...
	pr_debug("L2 Register Dumping End.\n");
}
	...
	unsigned int bytes, l2_dma_transfer_direction_t direction,	bool intr_enable)
{
	...
	regval &= ~(L2_FRAC_DMA_LOW_ADDR_MASK | (3<<30));
...
static void l2_get_addr(u8 id, void __iomem ** bufaddr)
{
	if (id < L2_UART_BUFFER_INDEX) {	/* L2 common buffer */
		*bufaddr = ZMP_VA_L2MEM + L2_COMMON_BUFFER_OFFSET +
			id * L2_COMMON_BUFFER_LEN;

	} else if (id < L2_USB_HOST_BUFFER_INDEX) {	/* UART L2 buffer */
		*bufaddr = ZMP_VA_L2MEM + L2_UART_BUFFER_OFFSET + 
	...
	l2_dma_transfer_direction_t direction, bool intr_enable)
{
	...
	local_irq_save(flags);
	//去掉l2_dma capture中的invalidate和clean cache的操作。
			...
			if (l2_frac_started && !frac_dmapending) {
				...
				case MEM2BUF:
					if (l2_dma_info[i].dma_frac_data_len <= 60)
						__raw_writel(0x0, ZMP_VA_L2MEM + i * 512 + 0x1FC);
					break;
				case BUF2MEM:
